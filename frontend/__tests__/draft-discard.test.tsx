...
describe("R11: mutation 失敗 (commit 未呼び出し) では draft が破棄されない", () => {
  ...
  it("TicketTagManager: 承認後にタグ色 (state-only draft) だけ変えても commit で破棄されない (R13)", () => {
    render(<TicketTagManager ticketId={TICKET_ID} currentTags={[]} allTags={[TAG_A]} />);
    fireEvent.click(screen.getByText("+ 新しいタグを作成"));
    const nameInput = screen.getByLabelText("新しいタグ名") as HTMLInputElement;
    fireEvent.change(nameInput, { target: { value: "draft-tag" } });
    ...
    // 他領域 mutation を承認・捕捉 (タグ色は hidden input 経由で signature に乗る)
    vi.spyOn(window, "confirm").mockReturnValue(true);
    let commit: () => void = () => {
      throw new Error("commit not captured");
    };
    act(() => {
      const r = prepareDiscardOnCommit();
      expect(r.approved).toBe(true);
      commit = r.commit;
    });

    // 承認後に色だけ変更 (ColorPicker の radio button = state-only draft)
    const radios = screen.getAllByRole("radio");
    const nextColor = radios.find((r) => r.getAttribute("aria-checked") === "false");
    if (nextColor) fireEvent.click(nextColor);

    act(() => commit());

    // signature 不一致 (色が hidden input に反映され変化) → 破棄されず dirty 残存
    expect(guard.dataset.dirty).toBe("true");
    expect(nameInput.value).toBe("draft-tag");
  });
