...
describe("loadTicket (Codex B2b R2/R3/R4 contract)", () => {
  it("by-id endpoint で解決し、所有 project の slug を project_slug に詰める", async () => {
    mockFetch.mockImplementation(async (path: string) => {
      if (path === "/api/v1/me/projects") return PROJECTS;
      if (path === "/api/v1/projects/p-aaa/tickets/tk-1") throw new BackendApiError(404, "nf");
      if (path === "/api/v1/projects/p-bbb/tickets/tk-1") return ticketPayload("tk-1");
      throw new BackendApiError(500, "unexpected");
    });

    const result = await loadTicket("tk-1");
  ...
  it("全 project が 404 のときだけ null を返す (notFound 経路)", async () => {
    ...
    await expect(loadTicket("missing")).resolves.toBeNull();
  });
  ...
  it("非 404 失敗は found より優先して rethrow する (fail-closed、R4)", async () => {
    ...
    mockFetch.mockImplementation(async (path: string) => {
      if (path === "/api/v1/me/projects") return PROJECTS;
      if (path === "/api/v1/projects/p-aaa/tickets/tk-1") throw new BackendApiError(403, "forbidden");
      if (path === "/api/v1/projects/p-bbb/tickets/tk-1") return ticketPayload("tk-1");
      throw new BackendApiError(500, "unexpected");
    });

    await expect(loadTicket("tk-1")).rejects.toMatchObject({ status: 403 });
  });
  ...
  it("所有 project の 5xx を false 404 に潰さず rethrow する", async () => {
    mockFetch.mockImplementation(async (path: string) => {
      if (path === "/api/v1/me/projects") return PROJECTS;
      if (path === "/api/v1/projects/p-aaa/tickets/tk-1") throw new BackendApiError(404, "nf");
      if (path === "/api/v1/projects/p-bbb/tickets/tk-1") throw new BackendApiError(500, "server");
      throw new BackendApiError(500, "unexpected");
    });

    await expect(loadTicket("tk-1")).rejects.toMatchObject({ status: 500 });
  });

  it("/me/projects 取得失敗は null に潰さず propagate する", async () => {
    mockFetch.mockImplementation(async (path: string) => {
      if (path === "/api/v1/me/projects") throw new BackendApiError(401, "unauth");
      return ticketPayload("tk-1");
    });

    await expect(loadTicket("tk-1")).rejects.toMatchObject({ status: 401 });
  });
});
