...
export function KeyboardReadinessStrip({
  ...
  return (
    <nav
      ...
      className="rounded-md border border-line bg-slate-50 dark:bg-slate-800 p-3"
    >
          ...
          {KEYBOARD_NAV_ITEMS.map((item) => (
            <li key={item.href}>
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-md border border-line bg-panel px-3 py-2 text-xs font-semibold text-ink outline-offset-2 hover:border-accent hover:text-accent focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent"
                href={item.href as Route}
              >
                {/* P0.1+ shortcut hint (planned; no key handler attached yet, F-P2R1-013) */}
                <kbd aria-hidden="true" className="rounded bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground">
                  {item.keys.join(" ")}
                </kbd>
...
export function AgentRunStateGraph() {
  return (
    <div className="grid gap-4">
      <div className="rounded-md border border-line bg-slate-50 dark:bg-slate-800 p-3">
          ...
          {AGENT_RUN_STATES_16.map((state, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded bg-panel/70 px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.25fr)_minmax(16rem,0.75fr)]">
        ...
        <section aria-labelledby="exception-transitions" className="rounded-md border border-line bg-panel p-3">
  ...
  return (
    <ul aria-label="blocked_reason fixed sub categories" className="grid gap-2 md:grid-cols-3">
      {BLOCKED_REASONS_3.map((reason) => (
        <li key={reason} className="rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-3">
    ...
    <ol aria-label="Chronological AgentRunEvent timeline" className="grid gap-2">
      {AGENT_RUN_EVENT_TIMELINE.map((event) => (
        <li
          ...
          className="grid gap-3 rounded-md border border-line bg-panel p-3 md:grid-cols-[4rem_minmax(10rem,0.8fr)_minmax(0,1fr)] md:items-start"
        >
  ...
  return (
    <dl className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
      {CONTEXT_SNAPSHOT_COLUMNS.map((column) => (
        <div key={column.key} className="rounded-md border border-line bg-panel p-3">
          <dt>
  ...
  return (
    <ul className="grid gap-2 md:grid-cols-3">
      {POLICY_PROFILES.map((profile) => (
        <li key={profile.name} className="rounded-md border border-line bg-panel p-3">
...
export function SecretBoundaryNotice({ title = "SecretBroker boundary" }: { title?: string }) {
  return (
    <section aria-label={title} className="rounded-md border border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-950/40 p-3">
...
function stateToneClass(state: AgentRunState): string {
  if (state === "blocked") {
    return "border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40";
  }

  if (TERMINAL_STATES.includes(state)) {
    return "border-teal-200 dark:border-teal-800 bg-teal-50 dark:bg-teal-950/40";
  }
