...
const STATUS_COLORS: Record<string, string> = {
  running: "border-amber-400 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40",
  completed: "border-emerald-400 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/40",
  failed: "border-red-400 dark:border-red-700 bg-red-50 dark:bg-red-950/40",
  cancelled: "border-gray-400 dark:border-gray-600 bg-gray-100 dark:bg-gray-800",
  blocked: "border-orange-400 dark:border-orange-700 bg-orange-50 dark:bg-orange-950/40",
};
  ...
  return (
    <div className={depth > 0 ? "ml-6 border-l-2 border-line pl-4" : ""}>
      <div className={`inline-flex items-center gap-2 rounded-md border-2 px-3 py-1.5 text-xs ${color}`}>
        ...
        {node.role ? <span className="rounded bg-panel/60 px-1.5 py-0.5 text-[10px] font-medium">{node.role}</span> : null}
