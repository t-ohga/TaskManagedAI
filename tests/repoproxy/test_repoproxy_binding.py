...
def test_repo_proxy_public_create_signature_exposes_only_binding() -> None:
    params = inspect.signature(RepoProxy.create_draft_pr).parameters

    assert tuple(params) == ("self", "binding")
    assert "request" not in params


def test_mock_repo_proxy_public_create_signature_exposes_only_binding() -> None:
...
def test_build_draft_pr_request_from_server_state() -> None:
    ...
    assert result.artifact_hash == DIFF_HASH
    assert result.policy_version == POLICY_VERSION
    assert result.provider_request_fingerprint == PROVIDER_FINGERPRINT
...
def test_binding_contains_only_ids() -> None:
    binding_fields = set(DraftPRBinding.__dataclass_fields__)

    assert binding_fields == {"tenant_id", "approval_id", "agent_run_id"}
