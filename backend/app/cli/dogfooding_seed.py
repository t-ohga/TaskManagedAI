...
async def _query_existing_dogfooding_tickets(
    ...
    stmt = select(Ticket).where(
        ...
        Ticket.slug.like("dogfooding-sprint-%"),
    )
...
async def _run(dry_run: bool) -> int:
    ...
    if not paths:
        print(f"WARNING: no Sprint Pack found in {_SPRINTS_DIR}", file=sys.stderr)  # noqa: T201  # CLI output
        return 1

    parsed: list[SprintPackMeta] = []
    ...
    print(f"Discovered {len(paths)} Sprint Pack files, {len(parsed)} parsed.")  # noqa: T201
    if parse_failures:
        print(f"Parse failures (frontmatter missing): {parse_failures}", file=sys.stderr)  # noqa: T201
    ...
    try:
        async with session_factory() as session:
            async with session.begin():
                report = await seed_sprint_packs(
                    session,
                    sprint_packs=parsed,
                    dry_run=dry_run,
                )
    ...
    print(f"{mode}: {report.to_dict()}")  # noqa: T201
    if report.failures:
        print(f"FAILURES: {report.failures}", file=sys.stderr)  # noqa: T201
        return 1
    return 0
