...
| Infrastructure | PostgreSQL、Redis、Docker runner、GitHub App、provider SDK、LocalSecretStore (Phase 0) / SOPS + age (D-4) | 永続化、queue、外部 API、runner、secret_ref 解決 | Domain contract を実装 |
...
| secret store | backend = `local` (LocalSecretStore、Phase 0 default) \| `sops` (SOPS + age、D-4)、ADR-00058 |
...
| Secrets | LocalSecretStore (Phase 0 default) / SOPS + age (D-4) + SecretBroker (backend=`local|sops`、ADR-00058) | secret_ref 抽象化と backend / Vault 移行余地を両立する |
