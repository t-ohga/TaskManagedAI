...
- Quality Loop lifecycle は **別 enum + 別 table** で表現 (P0.1 **SP-029 候補**で schema 化、本 run では doc-only artifact concept のみ。**SP-018 は既存 Hermes/memory sprint に予約済**のため使用不可、`docs/設計検討/修正まとめ統合計画.md` §10.4 + §12 で SP-029 が正式予約済、F-PR12-005 反映)
...
- **SP-029 候補** (P0.1、Quality Loop product artifact schema、本 update の §13.3 物理分離宣言の実装先、F-PR12-005 反映で SP-018 から変更)
...
defer_entry:
  ...
  target_artifact_hash: string (F-PR13-R7-002 P2 adopt: defer 対象 artifact の sha256、SP-012 §Server-owned AcceptanceArtifactBuilder の binding pattern と整合、defer resume 時の verifiability 確保)
  target_source_set_hash: string nullable (複数 source artifact の集合 hash、Decision approval policy F-P2R2-004 と整合)
```

既存 Sprint Pack 自由文 defer entry の structured 化は P0.1 SP-029 候補 accepted 後の別 run。
...
1. **最新 review chain (= `revision` linked to current artifact の `review` / `rereview`) の `verdict='clean'`** (findings: [] または `P3` / `info` のみで explicit accept) **OR** 全 finding が **fully resolved** = `adoption_decision='adopt'` + actual fix commit referenced / `adoption_decision='reject'` + acknowledgement rationale / `adoption_decision='defer'` + full `defer_entry` schema (resume_condition + verification + target_artifact_hash 全記入済) のいずれか (F-PR13-R7-006 P2 adopt: 「adoption_decision 記録のみ」では不十分、resolved status 必要)。historical R1/R2 は append-only history として保持、gate 対象外。F-PR13-002 P1 adopt 反映。
2. **全 `defer_entry` の `verification` 列が記入済** (severity を問わず、`adoption_decision='defer'` で close した finding 全件で `verification` 義務、F-PR13-R2-002 P2 adopt 反映)。`P3` / `info` finding を explicit accept する場合も同様、severity enum P0/P1/P2/P3/info の語彙統一 (F-PR13-004 P2 adopt)。P0/P1/P2 finding を `defer` する場合は **resume_condition と verification の両方が記入済** が必要 (gate を緩めない、blocking severity finding の defer 経路を fail-closed で防ぐ)
...
| 関連 table | `review_artifacts` (Phase C proposed) | Quality Loop artifact table (本 design doc proposed、SP-029 候補で実装) |
...
- **SP-029 候補 (P0.1、新規 Pack 起票必須)**: Quality Loop product artifact schema 実装、本 §14 + 別 design doc を core spec として cross-reference
- **ADR-00028 候補 (P0.1、proposed 新規起票必須)**: Quality Loop schema design (DB / API / event_type、ADR Gate Criteria #2/#3 trigger)
- ADR-00009 §QL-B update (本 §14.1 の `plan` / `review` / `revision` 等の生成は通常の `task_write` action として扱う)
- ADR-00014 (Multi-Agent Orchestration、**proposed**、F-PR13-R5-002 P3 adopt: 実 file `docs/adr/00014_multi_agent_orchestration.md:status=proposed` confirm): Phase C `review_artifacts` table と本 §14.4 物理分離宣言、ADR-00014 が P0.1 で accepted 化される時に本 cross-reference が有効化される
