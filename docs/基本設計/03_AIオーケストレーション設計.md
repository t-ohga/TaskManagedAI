...
1. 全 `review` / `rereview` artifact の `verdict='clean'` (各 round で findings: [] または LOW のみで explicit accept)
2. `defer_entry` の `verification` 列が記入済 (LOW finding を `defer` する場合)
