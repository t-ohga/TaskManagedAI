...
`policy_decisions` row 作成と action 実行は **同一 transaction** で完結し、`policy_decisions.created_at` < action audit event `created_at` を保証する。AgentRunEvent + audit event に `policy_decision_id` を必ず embed する (raw secret 除外)。

**実装は本 run 外**、各 Sprint Pack の acceptance spec として記録するのみ。
...
- Quality Loop lifecycle は **別 enum + 別 table** で表現 (P0.1 SP-018 候補で schema 化、本 run では doc-only artifact concept のみ)
...
- AgentRun.status 16 状態: DB CHECK (`migrations/versions/0007_agent_runs.py`) / SQLAlchemy CheckConstraint / Python Literal / Pydantic / pytest EXPECTED constant / frontend TypeScript enum (Sprint 9+)
- blocked_reason 3 種: 同上の 5+ source 整合 (drift 防止)
...
- SP-018 候補 (P0.1、Quality Loop product artifact schema、本 update の §13.3 物理分離宣言の実装先)
