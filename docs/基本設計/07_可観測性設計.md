...
- Sprint Pack 単位の lifecycle event chain: **`correlation_id=qloop-<sprint_pack_id>-<artifact_kind>-<seq>` 形式に固定** (F-PR13-R5-003 P2 adopt: 命名規則を §14.2 末尾の SP-023 future 文と統一、本 §14.2 と §14.4 クエリ例で同 format に sticks)。全 Quality Loop artifact 横串 trace では prefix match (`correlation_id =~ "qloop-SP-013-.*"`) を使う
...
**`correlation_id` 命名規則 (F-PR13-R5-003 P2 adopt 反映で固定)**: `qloop-<sprint_pack_id>-<artifact_kind>-<seq>` 形式 (e.g., `qloop-SP-013-plan-001` / `qloop-SP-013-review-001` 等)。本 format は P0.1 SP-023 候補 accepted 後の implementation でも維持、本 §14 内で sticks する。
