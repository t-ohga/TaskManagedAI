...
### 14.1 audit event 拡張候補 (P0.1 SP-023 候補で実装)
...
これら event_type 追加は ADR Gate Criteria #3 (API 契約 / event schema) trigger、5+ source 整合 (DB CHECK / SQLAlchemy / Literal / Pydantic / pytest / frontend) は SP-023 候補で実装。**本 run では doc-only、event_type enum に追加しない**。

### 14.2 correlation_id 規約 (§5 拡張)

Quality Loop artifact は Sprint Pack lifecycle event を表現するため、correlation_id 規約は本 §5 の延長として:

- Sprint Pack 単位の lifecycle event chain: `correlation_id=<sprint_pack_id>` で全 Quality Loop artifact 横串 trace
...
具体的な `correlation_id` 命名規則は SP-023 候補 で確定 (e.g., `qloop-<sprint_pack_id>-<artifact_kind>-<seq>`)。

### 14.3 metrics 拡張候補 (本 §7 拡張、P0.1 SP-023 候補で実装)
...
これらは P0.1 SP-023 候補で実装、本 §7.5 (`acceptance_pass_rate` / `time_to_merge` / `approval_wait_ms` / `citation_coverage` / `cost_per_completed_task`) の Quality KPIs 5 とは別 layer (Quality Loop 内部効率 metrics)。

### 14.4 Loki クエリ例 (本 §9 拡張、P0.1 SP-023 候補で event_type 追加後に有効化、F-PR13-003 P2 adopt 反映)
...
```
# Sprint Pack 単位で全 Quality Loop event を時系列追跡
{service="taskhub"} | json | event_type =~ "quality_loop_.*" | correlation_id = "qloop-SP-013-..."

# Codex multi-round の round 数集計
sum by (sprint_pack_id) (
  count_over_time({service="taskhub"} | json | event_type =~ "quality_loop_review_created|quality_loop_rereview_created" [7d])
)

# harness incident 発生時の前後 30 分の AgentRunEvent も併せて表示
{service="taskhub"} | json | event_type = "quality_loop_harness_incident_recorded" | source_artifact_id != ""
```

**Loki label 規約**: `event_type` / `correlation_id` / `sprint_pack_id` は **high cardinality field** のため Loki stream label に promote しない (`service` / `env` 等 low-cardinality label のみ stream selector に使う、本 §9 既存パターン準拠)。

これらクエリは P0.1 SP-023 候補で event_type 追加後に有効化される。

### 14.5 Grafana dashboard 拡張候補 (本 §10 拡張、P0.1 SP-023 候補で実装)
...
実装は SP-023 候補で SP-009 (UI Pack) 同時 update 経由。
...
- `plan` / `review` / `revision` / `rereview` artifact: 当該 Sprint Pack のライフタイム + 90 日 (Sprint Exit 後の retro / audit 用)
- `conformance` artifact: tenant 単位で **永続保存** (Sprint Exit evidence、`conformance.final_verdict` を AC-HARD/KPI trace に利用)
- `harness_incident` artifact: 90 日 (incident retrospective 用、Sprint Pack closed 後も保持)
...
- **SP-023 候補 (P0.1)**: Quality Loop event_type / metrics / Loki / Grafana 実装
- **ADR-00026 候補 (P0.1、proposed)**: Quality Loop schema design (DB / API / event_type)
- ADR-00007 (Observability、accepted): 本 §14 全体は ADR-00007 の延長として de facto accepted (event_type 追加は SP-023 + ADR-00026 で別 accepted)
