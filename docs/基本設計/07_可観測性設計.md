...
### 14.4 Loki クエリ例 (本 §9 拡張、P0.1 SP-023 候補で event_type 追加後に有効化)

Quality Loop event を Loki で trace する想定クエリ例 (本 §9 の延長):

```
# Sprint Pack 単位で全 Quality Loop event を時系列追跡
{event_type=~"quality_loop_.*", correlation_id="qloop-SP-013-..."}

# Codex multi-round の round 数集計
sum by (sprint_pack_id) (count_over_time({event_type=~"quality_loop_review_created|quality_loop_rereview_created"}[7d]))

# harness incident 発生時の前後 30 分の AgentRunEvent も併せて表示
{event_type="quality_loop_harness_incident_recorded"} | json | source_artifact_id != ""
```
