...
| upstream | `127.0.0.1:3900` / `127.0.0.1:8000` |
...
| auth key | **§5.3 と DD-06 に従い SecretBroker backend で暗号化管理 (Phase 0 default=`local`、`secret://local/p0/tailscale-auth-key#v1`。SOPS は D-4 移行先、ADR-00058)**。private staging CI/E2E 自体は Sprint 11.5 / D-1 前提。GitHub Actions runtime には **ephemeral / one-time** auth key として API 経由で短命注入し、reusable key は禁止。job 終了後に device cleanup と CI log mask を実施 |
...
| secret store | LocalSecretStore (Phase 0 default) / SOPS + age (D-4) | Vault / KMS / tenant 別 store |
