...
| network | P0 は必要最小限。ToolAdapter は `network_access='none'` default、P0.1 で `allowlist` のみ検討 |
...
| `network_access` | `none` / `allowlist` / `internet` (`internet` は P0 deny) |
| `allowed_actions` | `web_fetch` / `docs_search` |
...
SecretAdapter は SOPS + age + SecretBroker を抽象化し、将来 Vault へ切り替えられる境界を提供する。
