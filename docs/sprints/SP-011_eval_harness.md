...
- **SP-009 `partial_skeleton` 維持** (2026-05-24 status hygiene): #224/#225 で backend route reconciliation、frontend raw-payload schema guards、backend/frontend enum drift contract tests が追加済。SP-009-5 split Pack は docs-only 起票済だが、golden E2E、DOM secret scan、PayloadDataClass/future AuditEventType registry drift、SP-009-5 実装が残るため `done` 昇格は時期尚早
