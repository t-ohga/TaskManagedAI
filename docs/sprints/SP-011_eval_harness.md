...
- **per-run structural validation**: run_id (UUID v4 hex pattern + UUID() parseability)、tenant_id (non-bool int >= 1)、project_id (non-bool int >= 0)、status (AgentRun 16 状態 enum)、cost_usd (non-bool finite float >= 0.0) を strict 検査
