...
| PE-F-011 | SP-018 | ✅ trace added in `SP-018_hermes_memory_integration.md` (plan-only gate; implementation pending ADR acceptance) | SP-018 exit gate | Phase F の最初に ADR-00014/00018/00019 を Phase C R4 方針へ patch、action_class は ADR-00009 7 種以外を許さない strict CI、enum 交差禁止 test |
| PE-F-012 | SP-018 | ✅ trace added in `SP-018_hermes_memory_integration.md` (plan-only gate; implementation pending ADR acceptance) | SP-018 exit gate | agent_runs trigger 拡張 (before insert or update of tenant_id/project_id/role_id/role_scope)、agent_run_project_roles link table + role_scope=global の DB CHECK |
| PE-F-013 | SP-018 | ✅ trace added in `SP-018_hermes_memory_integration.md` (plan-only gate; implementation pending ADR acceptance) | SP-018 exit gate | sealed guard に追加 path (remote_agent adapter/router/frontend/config/tests)、P0.1 stub は remote_agent_dispatch_denied audit payload schema 定義 |
