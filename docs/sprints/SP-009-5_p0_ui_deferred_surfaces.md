...
- [x] Newcomer Path は F0 計画で read-only first route と F2+ API/schema/runtime gate に分割され、F2a で response-only dry-run API contract を固定し、F2b で `/api/v1/onboarding/dry_run_plan` を response-only backend API として実装し、F3 で `/onboarding` に dry-run form と plan-review rendering を追加し、初回利用者が理解前に mutating run を開始できない境界を固定した。
