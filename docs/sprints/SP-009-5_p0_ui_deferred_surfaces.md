...
| F2a | Guided intake dry-run contract plan (completed 2026-05-24) | response-only dry-run endpoint contract、no persistence |
| F2b | Guided intake dry-run backend implementation (completed 2026-05-24) | no implicit AgentRun start、API/schema gate 必須 |
| F3 | Plan review surface (completed 2026-05-24) | dry-run result rendering のみ、approve/start execution なし |
| F4 | CLI onboarding parity notes/tests (completed 2026-05-25) | `tm` canonical、ambiguous mutating command は fail-closed |
| F5 | Newcomer Path closeout (completed 2026-05-25) | route parity、browser/Vitest/contract verification、docs sync |
...
- [x] Newcomer Path は F0 計画で read-only first route と F2+ API/schema/runtime gate に分割され、F2a で response-only dry-run API contract を固定し、F2b で `/api/v1/onboarding/dry_run_plan` を response-only backend API として実装し、F3 で `/onboarding` に dry-run form と plan-review rendering、F4 で `tm` canonical CLI parity を追加し、初回利用者が理解前に mutating run を開始できない境界を固定した。
- [x] Newcomer Path F5 closeout で `/onboarding` route、`/api/v1/onboarding/dry_run_plan` API、`tm context show` / `tm doctor` / `tm ticket intake --guided` / `tm run plan --dry-run` CLI parity、response-only/no-persistence decision、GitHub inline review clean を同期した。
