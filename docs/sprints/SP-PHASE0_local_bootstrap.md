...
| credential-file 読取境界 (host-ambient exfiltration 防止) | ⏸ Phase 2 re-scope 待ち | per-agent 専用HOME + launcher tool-sandbox の AI/tool read-deny + exfiltration negative test は **CLIAgentAdapter の autonomous CLI 実行 (Phase 2) 前提**で Phase 0 単体では実装・検証不能 → §Review gate C 参照 (当初 must_ship だが Phase 2 へ re-scope する判断は user 承認待ち、未充足のまま completed にしない) |
...
- **host-ambient credential-file 読取境界**: per-agent 専用最小 HOME (該当 CLI の auth file のみ、他 secret 非同居) + launcher tool sandbox が credential file path への AI/tool 経由 read を deny (CLI 本体の auth 読取は許可)。**credential-file exfiltration negative test** (`cat ~/.codex/auth.json` 相当が deny、raw token が artifact/stdout に残らず canary 検出) PASS。残 exfiltration risk は accepted HIGH (local 単一 user) として記録、confidential 以上 payload の自律実行は Phase 2+ gate。
...
- **status: in_progress 維持 (completed にしない)**。実装 deliverable (S1-S4) + 自動検証 + runbook は完遂したが、下記 §completion gate の **gate C (credential-file exfiltration negative test)** が Phase 0 must_ship / exit 条件として残るため、over-claim を避け completed にしない (Workflow governance review HIGH adopt、2026-06-21)。実装は **検証可能な段階に到達**済、completed 化は gate C の scope 判断 (user) 待ち。
- **completion gate の現況 (正直化)**:
  ...
  - **gate C: credential-file exfiltration negative test** ❌ **未充足 (Phase 0 must_ship、completed の blocker)**: 受け入れ条件 (must_ship「credential-file 読取境界」/ 検証 §95 / ADR-00058 §exit) は「per-agent 専用最小 HOME + launcher tool sandbox の AI/tool 経由 credential-file read deny + `cat ~/.codex/auth.json` 相当 deny の **negative test PASS** + canary 検出」を Phase 0 exit に必須化する。**batch-3 の 34 検証 test に本 negative test は含まれず、per-agent 専用 HOME / CLI subprocess 内 read-deny も未実装** (launcher.py の `_FORBIDDEN_PATH_FRAGMENTS` は caller-supplied I/O path のみ deny し、CLI subprocess 内 AI/tool の exfiltration は対象外)。これらは **CLIAgentAdapter の autonomous CLI 実行 (Phase 2)** が前提のため Phase 0 単体では実装・検証不能。→ **gate C を Phase 2 へ re-scope する判断 (user 承認) があれば completed 可**。それまで in_progress 維持。
- **defer / follow-up (tracked)**: ① R16-F1 redeem transaction 境界 → ADR-00060 (proposed、Phase 2 着手 gate)。② secret-revoke approval target-binding (SecretRevokeApprovalClaim) → Phase 2 hardening。③ claude launchable agent entry → Phase 2 (CLIAgentAdapter)。④ generic ADR-00020 accept → AI framework intake track (ADR-00014/16 + P0完了 依存、別件)。⑤ gate C (credential-file exfiltration negative test + per-agent HOME + tool-sandbox read-deny) → Phase 2 (CLIAgentAdapter 前提)、user の re-scope 承認待ち。⑥ SP-001-5 は in_progress 維持 (DB/Redis internal-only vs restore 契約 reconciliation が host-phase の user/ADR 決定事項)。
