...
- **credential 供給** (ADR-00058、境界批評 finding-2 反映): **Phase 0 は CLI=host-ambient のみ** (host worker が `~/.claude`/`~/.codex` 直読、broker 非経由)。broker-managed (static API key) は **broker-owned spawn primitive** (broker 内部が resolve→inject→spawn→`assert_no_raw_secret`→zeroize を所有し result のみ返す、外部 callback に raw を渡さない、SecretHandle-only invariant 不変) で実装するが **Phase 2 (API transport 配線時)**。Phase 0 は primitive の設計確定のみ。
...
- **secret revoke** (ADR-00059、境界批評 finding-4 反映): deprecated→revoked 2 step、material 削除は status=revoked 後の別 step。**crash-safe**: `material_purged_at`/`purge_attempts` 列で durable に追跡し、`status='revoked' AND material_purged_at IS NULL` を idempotent purge する `secret gc-orphans` reconciliation。「revoked=削除済」は material_purged_at non-NULL で初めて真。rollback=再登録、DESTRUCTIVE approval gate。

## 実装チケット (4 stream)

| stream | 内容 | group | depends_on |
|---|---|---|---|
| **S1** secret tooling コア | uri_pattern.py 単一定数 / LocalStoreResolver / CompositeSecretResolver / SecretRefRepository.create / SecretRegistrationService / host-ambient CLI 供給 (broker 非経由) / revoke 2 step + durable reconciliation。broker-owned spawn primitive は設計のみ (Phase 2 実装) | G1 (先行) | ADR-00058 accepted |
...
- S1: `uri_pattern.py` 単一定数 (`secret://(sops|local)/...`) → `local_store_resolver.py` (keyring+Fernet, canary redaction 再利用, raw 内部のみ) → `resolver_dispatch.py` (CompositeSecretResolver, backend dispatch, 未知 fail-closed) → `secret_ref.py` create/insert (metadata-only, server-owned uri 組立, active≤1/pending≤1) → `registration.py` (raw を store へ即委譲, DB commit 後 store 順序固定, self-rotating を broker-managed 登録 reject) → **host-ambient CLI 供給** (host worker が file 直読、broker 非経由) → revoke 2 step + durable reconciliation (`material_purged_at` 列 + `gc-orphans`)。**broker-owned spawn primitive は設計確定のみ、実装は Phase 2**。
- S2: migration 0049 (down_revision=0048, regex は uri_pattern.py と文字列一致) + ORM CheckConstraint 同期 + downgrade `secret://local/%` row preflight + `secret://sops/` 後方互換 test。
...
| broker-owned spawn primitive (API key 注入、設計確定) | ✅ (設計) | 実装は Phase 2 |
...
- **境界批評 finding-2 解消**: Phase 0 の CLI 供給は host-ambient (host worker file 直読、broker 非経由、worker env に token 非常駐)。broker-owned spawn primitive (API key、Phase 2) は broker 内部が resolve→inject→spawn→scan→zeroize を所有し result のみ返す設計を ADR-00058 で確定 (外部 callback に raw を渡さない、SecretHandle-only 不変)。Phase 0 host-ambient test で worker env 非常駐・self-rotating broker-managed 登録 reject 確認。
...
- revoke が deprecated→revoked 2 step (active→revoked 直行なし)、material 削除は revoked 後の別 step。crash-window test (DB revoked 後 store.delete 前 crash → `material_purged_at IS NULL` 残置 → `gc-orphans` で idempotent 収束) PASS。
- URI backend 5+source 整合 (drift guard test で `SECRET_URI_PATTERN` の exact 一致)、未知 backend fail-closed、既存 `secret://sops/` 後方互換。migration 0049 両方向 (downgrade `secret://local/%` row preflight、alembic check PASS)。
...
- loopback bind regression。revoke 2 step + durable reconciliation (material_purged_at + gc-orphans、crash-window)。
