...
|---|---|---|---|
| **S1** secret tooling コア | uri_pattern.py 単一定数 / LocalStoreResolver / CompositeSecretResolver / SecretRefRepository.create / SecretRegistrationService / host-ambient CLI 供給 (broker 非経由) / revoke (rule §5: active/deprecated/pending→revoked) + durable reconciliation (material_reconciliation + gc-orphans)。broker-owned spawn primitive は設計のみ (Phase 2 実装) | G1 (先行) | **ADR-00058 + ADR-00059 accepted** (revoke/material は ADR-00059 gate) |
...
- S2: migration 0049 URI backend CHECK (**revision 固定 SQL literal を hardcode、`SECRET_URI_PATTERN` を import しない** = migration 不変性。drift guard test で「最新 migration の固定 literal == current `SECRET_URI_PATTERN`」を CI 強制) + **migration 0050 material_purge_tracking** (`material_purged_at timestamptz NULL` / `purge_attempts int` additive 列、ADR-00059 revoke durability 用、条件付き downgrade preflight `status='revoked' AND material_purged_at IS NULL` 0 件 + crash-window test) + ORM CheckConstraint は `SECRET_URI_PATTERN` を import (runtime source) + downgrade `secret://local/%` row preflight + `secret://sops/` 後方互換 test + **canonical 正本同期** (`.claude/rules/secretbroker-boundary.md §2` + `.claude/reference/secretbroker-contract.md §2` + `.claude/rules/core.md` + `.claude/reference/testing.md` + `.claude/agents/taskmanagedai/{code-reviewer,security-specialist}.md` を `secret://(sops|local)/...` へ additive 更新、5+source の rule/reference/agent 源を漏れなく含める)。
