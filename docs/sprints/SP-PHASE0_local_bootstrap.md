...
planned_adr_refs:
  - "ADR-00020 (Framework Intake Checklist: keyring 依存の license/attribution/SBOM intake)"
related_sprints:
...
| credential-file 読取境界 (host-ambient exfiltration 防止) | ✅ | per-agent 専用HOME + tool sandbox deny + canary、残 HIGH risk は Phase 2 gate |
...
- ADR-00058 (CLI 認証境界=案C hybrid) + ADR-00059 (loopback + revoke) が accepted、ADR-00020 (Framework Intake Checklist) で keyring 依存 intake。
...
- ADR-00020 (Framework Intake Checklist: keyring 依存 intake)。
...
- **status: ready → completed** (実装 deliverable 完遂)。
- **user handoff (実装外、別途実施)**: `docs/deploy/host-setup.md` の **clean Mac 実機 runbook walkthrough** (up→alembic upgrade head→seed→/healthz green→MCP stdio→host worker→`taskhub secret-create`) は Sprint Pack §119 の通り **user 実施** (CI/ローカルは config/DB レイヤを機械検証するが、実機の docker daemon + OAuth refresh + clean-Mac 再現性は operator 検証)。下記「残リスク / follow-up」参照。
- **defer / follow-up (tracked)**: ① R16-F1 redeem transaction 境界 → ADR-00060 (proposed、Phase 2 着手 gate)。② secret-revoke approval target-binding (SecretRevokeApprovalClaim) → S4/Phase 2 hardening。③ claude launchable agent entry → Phase 2 (CLIAgentAdapter)。④ SP-001-5 は in_progress 維持 (host-setup 実機検証 + DB/Redis internal-only vs restore 契約 reconciliation が host-phase の user/ADR 決定事項)。
