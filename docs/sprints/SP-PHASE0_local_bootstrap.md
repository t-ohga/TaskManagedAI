...
related_sprints:
  ...
  - "SP-006_cli_artifact"
risks:
  - "credential injection の broker 境界逆流 (CRITICAL、境界批評で fix 済設計を実装で堅持)"
  - "URI scheme 5+source drift (fail-open)"
...
現状は「記録」は自律で動くが「実行」が構造的に欠落 (大元計画 §1 の 5 大ブロッカー)。Phase 0 はその最下層 = 起動 + secret + CLI 認証供給を固める。実行エンジンは CLI 主 (worker が `claude -p` / `codex exec` をサブスク認証で headless spawn)。Phase 0 設計で **CLI サブスク credential が self-rotating OAuth token** と実機確認され、全 broker-managed (案B) が構造的に不可能 → host-ambient (案C hybrid) を user 承認 (2026-06-20)。境界批評が初期設計を REFUTE (broker が raw を launcher へ forward する seam は SecretBroker §10 違反) し、**broker-mediated operation callback 内 spawn** の修正版に到達。
...
- **credential 供給 = broker-mediated operation 反転** (ADR-00058、境界批評 CRITICAL fix): subprocess spawn を `broker.redeem_capability_token` の operation callback 内で実行。SecretHandle-only invariant 維持、raw は operation 内のみ materialize、caller 非返却。launcher signature `launch_cli_agent(request, registry)` 不変。
- **URI scheme local:// 拡張** (ADR-00058): 単一定数 `SECRET_URI_SCHEME_PATTERN` (uri_scheme.py) を DB CHECK/ORM/resolver/register/test が import (5+source drift 構造防止)、未知 scheme fail-closed。
...
- **secret revoke** (ADR-00059): deprecated→revoked 2 step、material 削除は status=revoked 後の別 step、削除失敗は orphan GC、rollback=再登録、DESTRUCTIVE approval gate。

## 実装チケット (4 stream)

| stream | 内容 | group | depends_on |
|---|---|---|---|
| **S1** secret tooling コア | uri_scheme.py 単一定数 / LocalStoreResolver / CompositeSecretResolver / SecretRefRepository.create / SecretRegistrationService / broker-mediated credential operation 反転 / revoke 2 step | G1 (先行) | ADR-00058 accepted |
| **S2** DB schema | migration 0049 (URI CHECK 拡張、両方向 lossless) + ORM 同期。S1 と密結合のため **S1+S2 を 1 PR** に統合 (cross-source-enum 単独 PR 方針) | G1 | S1, ADR-00058 |
...
| **S4** verification + 正本化 | test_compose_loopback_binding / e2e (broker operation 内 resolve) / credential subprocess injection / docker smoke (throwaway) / SP-001-5 §Review close / 進捗 doc | G3 (最終) | S1,S2,S3, ADR-00058/00059 |

## タスク一覧 (主要)

- S1: `uri_scheme.py` 単一定数 → `local_store_resolver.py` (keyring+Fernet, canary redaction 再利用, raw 内部のみ) → `resolver_dispatch.py` (CompositeSecretResolver, scheme dispatch, 未知 fail-closed) → `secret_ref.py` create/insert (metadata-only, server-owned uri 組立, active≤1/pending≤1) → `registration.py` (raw を store へ即委譲, DB commit 後 store 順序固定, self-rotating を broker-managed 登録 reject) → **broker-mediated credential operation 反転** (spawn を operation callback 内, SecretHandle-only 維持) → revoke 2 step + material 分離。
- S2: migration 0049 (down_revision=0048, regex は uri_scheme.py と文字列一致) + ORM CheckConstraint 同期 + downgrade local:// row preflight + sops:// 後方互換 test。
...
- S4: test_compose_loopback_binding (ports 撤回 regression guard) + e2e DB-gated (broker operation 内 resolve, negative: actor/run/fingerprint mismatch) + credential subprocess injection (raw 非常駐/argv 非露出/終了残留なし/signature 物理削除 negative) + docker smoke (専用 project/port/throwaway volume, 実運用 volume 非汚染) + SP-001-5 §Review close (status in_progress 維持) + 進捗 doc。
...
| broker-mediated credential operation 反転 (CRITICAL fix) | ✅ | - |
...
- **境界批評 CRITICAL 解消**: credential 供給が broker-mediated operation 反転で実装、SecretHandle-only invariant 維持、raw は operation 内のみ materialize/caller 非返却、launcher signature 不変。subprocess injection test で raw 非常駐・argv 非露出・終了残留なし確認。
...
- revoke が deprecated→revoked 2 step (active→revoked 直行なし)、material 削除は revoked 後の別 step + 失敗時 orphan GC。
- URI scheme 5+source 整合 (drift guard test で exact set 一致)、未知 scheme fail-closed、既存 sops:// 後方互換。migration 0049 両方向 (downgrade local:// row preflight、alembic check PASS)。
...
- loopback bind regression。revoke 2 step + orphan GC。
