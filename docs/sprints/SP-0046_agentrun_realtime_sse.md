...
- **(2026-06-04 台帳監査) status drift 訂正**: 機能は L-3 SSE として PR #301 (ADR-00038) で main merge 済だが frontmatter が `in_progress` のままだった。実態 (上記 Review = 実装完了 + R3 approve) に合わせ `completed` へ訂正。
