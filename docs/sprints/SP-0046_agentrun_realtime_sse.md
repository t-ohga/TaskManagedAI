...
4. T3 endpoint: **(a) `?last_event_id=` validation (非整数/UUID は 400/422) + tenant/actor は新設 sessionless dep で request state から解決 (yield 型 `get_db_session` / 既存 `get_current_actor_id` を dep graph に含めない、R6+R7 HIGH) → (b) run/active-scope preflight は明示的短命 session (`async with AsyncSessionFactory()`) で実行し response return 前に close (scope 外 404) → (c) custom ASGI streaming response を return**。custom response の `__call__` 内で **capacity 判定 (Semaphore/counter、満杯のみ 503+Retry-After を `__call__` から送出) → 空きあれば LISTEN connection 取得 (短い正の timeout、`timeout=0` 禁止) → 200 SSE header (`text/event-stream` + `X-Accel-Buffering:no` + `Cache-Control:no-cache`) → LISTEN → catch-up → stream (tail/status は per-query 短命 session) → finally release+slot 解放** を単一 try/finally に収める。flag-off / 恒久停止は 204。
