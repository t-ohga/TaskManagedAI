...
risks:
  - "Memory API is feature-flagged disabled by default; deployment opt-in must remain explicit"
  ...
  - "memory mutation UI / curator automation / cron routines remain deferred to SP-019/SP-020/P1+"
---

最終更新: 2026-05-24 (SP-018 completed closeout: batch 0a-0h merged)

## 目的

Hermes-agent 由来の memory / context pattern を TaskManagedAI 境界で再実装し、run history、review finding、user preference、repo cache を tenant/project scoped memory として扱えるようにする。SP-018 は **P1 memory backend の実装 Sprint** として、ADR readiness、DB schema、store/retrieval pipeline、ContextSnapshot guard、backup/restore drill、feature-flagged read-only API contract までを batch 0a-0h で完了した。
...
- `tm memory` runtime command enablement / deployment-side feature flag opt-in

## readiness gate

| gate | status | note |
|---|---|---|
| ADR-00016 accepted | completed | SP018-T01 で accepted 化し、runtime 実装は SP018-T02-T08 で完了 |
| ADR-00024 accepted | completed | SP018-T01 で accepted 化し、cross-project retrieval deny-by-default を service/schema/DB/test で固定 |
| content storage shape | completed | `content_artifact_ref` + `content_hash` + `sanitizer_version_id` を default として ADR-00024 / schema / service に同期 |
| artifacts project boundary | completed | `artifacts.project_id` + unique `(tenant_id, project_id, id)` prerequisite を使い、memory FK を project-scoped に接続 |
| sanitizer_policy_versions seed | completed | `sanitizer_policy_versions` FK + stored `config_hash` を memory record / retrieval path に接続し、drift は `stale_sanitizer` deny |
...
|---|---|
| PE-F-011 | memory prompt enrichment is ref-only / untrusted by default; no action_class expansion is introduced by SP-018. |
| PE-F-012 | memory records/retrievals keep tenant/project FK boundaries and do not widen agent role references. |
| PE-F-013 | remote agent adapter/router/frontend/config/test paths remain sealed; memory API adds only feature-flagged read-only retrieval and no remote dispatch path. |
...
### 2026-05-24 closeout: SP-018 completed
...
implemented:
- Frontmatter corrected from `ready` to `completed` after all SP018-T00-T08 tasks reached `[x]`.
- Sprint registry note updated from plan-only/blocked to completed memory backend.
- SP-022 Phase E trace updated so PE-F-011/012/013 no longer claims SP-018 implementation is pending ADR acceptance.

verified:
- PR #187: plan-only gate.
- PR #188: ADR readiness / accepted promotion.
- PR #189: schema foundation.
- PR #190: store pipeline.
- PR #191: retrieval pipeline.
- PR #192: ContextSnapshot retrieval guard.
- PR #193: backup/restore memory drill.
- PR #194: disabled memory API contract.

deferred:
- `tm memory` runtime enablement and deployment opt-in remain follow-up work.
- memory mutation UI, curator/insights automation, cron/routines remain SP-019/SP-020/P1+.

risks:
- Hosted GitHub Actions remain unavailable because of monthly quota; local verification and Codex review helper results were used as the quality signal for each PR.
