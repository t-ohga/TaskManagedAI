...
(2026-06-05 追記) human kill switch (2) は **[ADR-00048](../adr/00048_superintendent_emergency_stop.md) (proposed)** で設計化。codex-plan-review R1-R3 で計 11 設計欠陥 (CRITICAL 5 + HIGH 6) を捕捉し、**SP-035 在来実装 (agent = MCP stdio process 内 subprocess) のままでは FastAPI kill switch から cross-process に届かず壊れた安全機能になる**ことが判明。正しい実装は agent supervision の cross-process 化 (DB 永続 registry + supervisor/control channel) + provider postflight generation CAS + AgentRun state machine 改訂を伴う**専用 architecture sprint** が前提。よって kill switch 実装は architecture sprint に分離 (ADR-00048 = 設計正本)。auto-approve (1) は conservative-by-default を P0.1 安全 default として ADR-00048 で意図的 defer。
