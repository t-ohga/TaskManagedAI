...
- [ ] Codex prompt 作成 (batch 1-7 ごと、必要時に codex-all-loops mode=code 起動)
- [ ] BL-UII-001-007 順次実装 (Codex 委譲 + Claude adopt/reject/defer 判定 + write back)
- [ ] Vitest regression test PASS (各 batch 内で完結)
...
- [ ] navigation 全 item が日本語
- [ ] Ticket / Approval / AgentRun / Audit / Eval Dashboard 主要 page 全 label が日本語 (英語 placeholder / tooltip は除く)
- [ ] error / loading / 404 / empty state が日本語
- [ ] 技術用語は原語維持 + 括弧で日本語併記 (初出のみ、以降省略)
- [ ] enum 値の表示 label は日本語 (enum 値自体は不変)
- [ ] Vitest regression test PASS (各 page で日本語 label が表示確認)
