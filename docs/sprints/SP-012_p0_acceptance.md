...
### batch 7 (taskhub admin CLI skeleton: restore / migrate / status / age-rotate / verify / 2026-05-18 session)

#### Changed
- `scripts/taskhub_admin.py` 新規 (~240 LOC): ADR-00021 §3 host-portable admin CLI の subcommand + exit code contract skeleton
  - subcommands: `restore --input <path>.tar.age` / `migrate --target <host> [--via closed-network|scp]` / `status` / `age-rotate` / `verify --integrity|--network-invariant`
  - exit code contract: 0=clean / 1=skeleton mode (real I/O not implemented) / 2=CLI usage error (argparse error / missing required option / nonexistent input path)
  - real I/O (age decrypt / pg_restore / Redis import / SOPS re-encrypt / row count / checksum / alembic check) は user 物理 drill phase (ADR-00021 §8) に defer
- `tests/scripts/test_taskhub_admin.py` 新規 (~140 LOC、15 tests): subcommand routing / required option / nonexistent input / skeleton message / exit code 0/1/2 を contract test

#### Verified
- `uv run ruff check` PASS / `uv run mypy scripts/taskhub_admin.py` PASS / `uv run pytest tests/scripts/test_taskhub_admin.py -v` 15 passed
...
- 閉域ネットワーク不変: literal を排除 (hook `check-tailscale-grants.sh` を全 edit で PASS)、network invariant 言及は ADR-00007 reference に限定
...
- real backup / restore / migrate / age-rotate / verify I/O 実装: user 物理 drill phase (host migration / backup restore drill / age key rotation real exec)
...
- `--via` の transport 選択肢は skeleton placeholder (`closed-network` / `scp`)、real transport adapter は drill 設計時に確定
