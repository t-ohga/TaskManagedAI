...
planned_adr_refs:
  - "[ADR-00025](../adr/00025_autonomy_policy_profiles.md) # proposed; promote to accepted only after SP024-T01 readiness gate"
related_sprints:
...
最終更新: 2026-05-24 (SP024-T00 plan-only gate)
...
- ADR-00025 は proposed のまま `SP-024` を実装 Sprint として予約している。
...
- 現行 code は `ProjectCreate` で caller-supplied `policy_profile` を拒否し、repository payload でも `policy_profile` を server-owned として reject している。一方で `projects.policy_profile` は DB 上の server-owned field として存在するため、ADR-00025 の古い「列削除」記述は実装前に ADR drift として解消する必要がある。
...
| gate | status | note |
|---|---|---|
| ADR-00025 proposed | ready | SP024-T01 で accepted 化可否を判定する |
...
| `projects.policy_profile` compatibility decision | blocker for T02 | ADR-00025 の古い列削除案と ADR-00009 accepted schema の整合を T01 で確定する |
...
- **T01 resolves ADR drift before migration**: `projects.policy_profile` を削除するか、server-owned cache として維持するかは ADR-00009 compatibility を検証してから決める。default path は「列を維持し、`autonomy_level -> policy_profile` resolver で更新する」。
...
- SP024-T01: ADR-00025 readiness gate。ADR-00025 を current implementation と同期し、accepted 化可否、batch split、`projects.policy_profile` compatibility decision を確定する。
- SP024-T02: `autonomy_level` enum + DB migration。default L0、DB CHECK / SQLAlchemy / Python Literal / Pydantic / pytest / docs の 5+ source 整合を作る。
...
- [ ] SP024-T01 ADR-00025 readiness gate
- [ ] SP024-T02 autonomy_level enum + DB migration
...
- `projects.policy_profile` の扱いは ADR-00009 accepted 実装との互換性が高いため、T01 で削除案と維持案を比較しないまま migration へ進むと regression risk が高い。
