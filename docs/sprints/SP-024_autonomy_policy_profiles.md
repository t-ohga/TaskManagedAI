...
最終更新: 2026-05-24 (SP024-T08 regression / rollback closeout completed)
...
- SP024-T06: audit / AgentRunEvent / policy_decisions trace (completed)。auto-allow path でも `policy_profile` / `policy_version` / `auto_allow_reason` / `effective_action_class` / `applied_level` を raw payload なしで記録する。
- SP024-T07: UI / CLI autonomy setting (completed)。`autonomy_level` だけを表示・更新し、`policy_profile` の setter / input / command option を作らない。
- SP024-T08: regression / rollback gates (completed)。migration up-down、enum drift、caller payload reject、human-required action、kill switch override、14 row seed維持をまとめて close する。
...
- [x] SP024-T06 audit / AgentRunEvent / policy_decisions trace
- [x] SP024-T07 UI / CLI autonomy settings
- [x] SP024-T08 regression / rollback closeout
...
### 2026-05-24 SP024-T06 audit / AgentRunEvent / policy_decisions trace

changed:
- `backend/app/services/policy/autonomy_trace.py`
- `backend/app/services/policy/__init__.py`
- `tests/policy/test_autonomy_trace.py`
- `docs/sprints/SP-024_autonomy_policy_profiles.md`
- `docs/adr/00025_autonomy_policy_profiles.md`

implemented:
- `append_autonomy_policy_trace()` / `build_autonomy_policy_trace_payloads()` を追加し、1 つの `AutonomyPolicyEngineDecision` から `policy_decisions`、AuditEvent `policy_decision_created`、AgentRunEvent `policy_linted` の 3 ledger payload を生成・append する境界を追加。
- trace API は raw prompt / raw provider payload / raw secret / capability token を受け取らず、`input_hash` (lowercase SHA-256) と redacted decision summary のみを記録。
- `metadata_` に `applied_level`、`effective_action_class`、`auto_allow_reason`、`policy_engine_reason_code`、`profile_reason_code`、`low_risk_failed_axes`、`override_source`、`required_review_artifact` を記録。
- auto-allow かつ review artifact required の場合は `required_review_artifact_id` を必須化し、review artifact なしの allow trace を reject。

verified:
- `uv run ruff check backend/app/services/policy/autonomy_trace.py backend/app/services/policy/__init__.py tests/policy/test_autonomy_trace.py`
- `PYTHONPATH=cli uv run mypy backend/app/services/policy/autonomy_trace.py backend/app/services/policy/__init__.py tests/policy/test_autonomy_trace.py`
- `uv run pytest tests/policy/test_autonomy_trace.py tests/policy/test_autonomy_policy_engine.py -q`

deferred:
- Runtime caller への接続、settings endpoint、UI / CLI write surface は SP024-T07。

risks:
- T06 は trace append boundary まで。実 runtime path へ接続する T07 では、実 `review_artifacts` FK を持つ auto-allow path と idempotency key 方針を追加検証する必要がある。
...
### 2026-05-24 SP024-T07 UI / CLI autonomy settings

changed:
- `backend/app/api/me.py`
- `backend/app/services/policy/autonomy_settings.py`
- `backend/app/services/policy/__init__.py`
- `cli/tm/commands/settings.py`
- `cli/tm/commands/__init__.py`
- `frontend/app/(admin)/settings/page.tsx`
- `frontend/app/(admin)/settings/actions.ts`
- `frontend/lib/api/session.ts`
- `tests/api/test_me_api.py`
- `tests/api/test_me_autonomy_settings_contract.py`
- `tests/api/test_sp012_9_ui_wiring_routes.py`
- `tests/cli/test_tm_cli_foundation.py`
- `frontend/__tests__/settings-auth-common-i18n.test.tsx`
- `frontend/__tests__/lib/api/session.test.ts`
- `docs/adr/00025_autonomy_policy_profiles.md`
- `docs/sprints/SP-024_autonomy_policy_profiles.md`

implemented:
- `/api/v1/me/projects/{project_id}/autonomy` PATCH endpoint を追加し、request schema は `autonomy_level` のみ許可、extra field は forbid。
- `ProjectAutonomySettingsService` を追加し、generic `ProjectRepository` の server-owned reject 境界を維持したまま、狭い settings writer で `autonomy_level` を更新し、`policy_profile` は resolver 経由で server-owned に再解決。
- `tm settings autonomy --level L0|L1|L2|L3` を追加し、capability は `task_write`、request body は `{"autonomy_level": ...}` のみ。
- Settings UI に autonomy selector + 保存 action を追加し、frontend API client / server action でも `policy_profile` setter を作らない。

verified:
- `uv run ruff check backend/app/services/policy/autonomy_settings.py backend/app/services/policy/__init__.py backend/app/api/me.py tests/api/test_sp012_9_ui_wiring_routes.py tests/api/test_me_api.py tests/api/test_me_autonomy_settings_contract.py tests/cli/test_tm_cli_foundation.py cli/tm/commands/settings.py cli/tm/commands/__init__.py`
- `PYTHONPATH=cli uv run mypy backend/app/services/policy/autonomy_settings.py backend/app/services/policy/__init__.py backend/app/api/me.py tests/api/test_sp012_9_ui_wiring_routes.py tests/api/test_me_api.py tests/api/test_me_autonomy_settings_contract.py tests/cli/test_tm_cli_foundation.py cli/tm/commands/settings.py cli/tm/commands/__init__.py`
- `uv run pytest tests/api/test_sp012_9_ui_wiring_routes.py tests/api/test_me_autonomy_settings_contract.py tests/cli/test_tm_cli_foundation.py -q`
- `TASKMANAGEDAI_RUN_DB_TESTS=1 TASKMANAGEDAI_DATABASE_URL=<isolated 127.0.0.1:55434 test db> uv run pytest tests/api/test_me_api.py -q`
- `corepack pnpm@10.18.0 --dir frontend exec tsc --noEmit`
- `corepack pnpm@10.18.0 --dir frontend exec eslint 'app/(admin)/settings/page.tsx' 'app/(admin)/settings/actions.ts' lib/api/session.ts __tests__/settings-auth-common-i18n.test.tsx __tests__/lib/api/session.test.ts --max-warnings=0`
- `corepack pnpm@10.18.0 --dir frontend exec vitest run __tests__/settings-auth-common-i18n.test.tsx __tests__/lib/api/session.test.ts`

deferred:
- runtime caller への trace append 接続と rollback/regression closeout は SP024-T08。

risks:
- T07 は settings write surface まで。L1-L3 runtime auto-allow enablement は引き続き Policy Engine / low-risk / trace gate 経由で扱い、settings endpoint 自体は auto-allow 実行を開始しない。

### 2026-05-24 SP024-T08 regression / rollback closeout

changed:
- `tests/policy/test_autonomy_regression_closeout.py`
- `docs/adr/00025_autonomy_policy_profiles.md`
- `docs/sprints/SP-024_autonomy_policy_profiles.md`
- `docs/sprints/README.md`

implemented:
- SP-024 closeout として Sprint Pack status を `completed` に更新。
- T08 専用 regression を追加し、L0-L3 allow matrix と human-required actions の disjoint、全 level の human approval fallback、global kill switch precedence、L0 downgrade semantics を固定。
- 既存 regression gate と合わせて、enum drift、caller payload reject、low-risk 7 axes、kill switch / provider / tool / budget override、14 row policy seed、schema introspection、migration downgrade/upgrade round-trip を再確認。

verified:
- `uv run ruff check tests/policy/test_autonomy_regression_closeout.py`
- `PYTHONPATH=cli uv run mypy tests/policy/test_autonomy_regression_closeout.py`
- `uv run pytest tests/policy/test_autonomy_regression_closeout.py -q`
- `uv run pytest tests/policy/test_autonomy_regression_closeout.py tests/policy/test_autonomy_policy_engine.py tests/policy/test_autonomy_profile_resolver.py tests/policy/test_low_risk_profile.py tests/policy/test_autonomy_level_enum.py tests/api/test_me_autonomy_settings_contract.py tests/cli/test_tm_cli_foundation.py -q`
- `TASKMANAGEDAI_RUN_DB_TESTS=1 TASKMANAGEDAI_DATABASE_URL=<isolated 127.0.0.1:55434 test db> uv run pytest tests/policy/test_policy_profile_seed.py tests/db/test_schema_introspection.py -q`
- `TASKMANAGEDAI_DATABASE_URL=<isolated 127.0.0.1:55434 test db> uv run alembic upgrade head && uv run alembic downgrade -1 && uv run alembic upgrade head`
- `TASKMANAGEDAI_DATABASE_URL=<isolated 127.0.0.1:55434 test db> uv run alembic current` -> `0034_sp024_autonomy_level (head)`

deferred:
- `uv run alembic check` remains blocked by existing infrastructure debt: `migrations/env.py` does not provide a MetaData object to autogenerate. This is not introduced by SP-024 and should remain a carry-over infrastructure task.

risks:
- SP-024 completes the policy/profile/settings/trace/regression foundation. Runtime caller integration and dogfooding enablement for L1-L3 auto-allow should be handled as a separate opt-in Sprint, not by silently changing SP-024 defaults.
