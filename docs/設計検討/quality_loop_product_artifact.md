...
Sprint Pack の `## 残リスク` / `## 次スプリント候補` / `## defer_if_over_budget` 列で使われる「`defer`」を **structured state** として表現する。現状は自由文で drift しやすいため、本 design doc で必須 **7 field** を定義する (F-PR13-R11-006 P2 adopt: 5 → 7 field、target_artifact_hash + target_source_set_hash を本文で明示):
  ...
  findings: [
    {
      ...
      evidence_quote: text (file:line の該当部分の **redacted** 引用、Codex finding の `body` から抽出。F-PR13-R11-007 P1 adopt: 保存前に raw secret / PII / canary value / provider key / capability token 値の redaction 必須、`.claude/rules/secretbroker-boundary.md §11` audit payload 規約 + `.claude/rules/provider-compliance.md §11` redaction 規約準拠。redaction pattern hit 種別は別 column で記録、raw value は保存しない),
      suggestion: text,
...
### 6.2 harness incident zero gate (F-PR13-R11-002 P2 adopt 反映: recovery_action vocabulary 統一)

Sprint Pack の Sprint Exit 時点で `quality_loop_harness_incident` artifact のうち **`resolved_at` が null** または **`recovery_action` が `rollback` / `abort` / `defer_entry_migrated` のいずれかに設定されていない** row が残っているなら、Sprint Exit を block:
