...
### 6.1 open finding zero gate

Sprint Pack の `conformance` artifact (本 design doc §2) を発行する条件:

- 該当 Sprint Pack の **全 `review` / `rereview` artifact の `verdict` が `clean`** (各 round で findings: [] または LOW のみで explicit accept)
- `defer_entry` で `verification` 列が記入済 (LOW finding を `defer` する場合)
...
### 6.3 clean evidence の verification
