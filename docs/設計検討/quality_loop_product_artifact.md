...
- **ADR-00026 候補 (P0.1、proposed 新規起票必須、F-PR13-R3-003 + R4-006 P2 adopt 強化)**: SP-023 と対応する Quality Loop schema ADR (DB / API / event_type)、本 PR commit 時点で `docs/adr/` 配下に ADR-00026 実 file 未起票 (R29 plan 上の予約のみ、ADR 番号 25/26 は本 ADR 用に reserved、ADR-00025 は本 QL-B PR #12 で proposed 起票済、ADR-00027 は QL-A で Tool Registry 用に proposed 起票済、ADR-00026 は本 SP-023 用に reserved)。本 design doc を background として cross-reference。
