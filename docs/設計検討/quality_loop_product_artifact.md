...
- **SP-023 候補 (P0.1、新規 Pack 起票必須、`docs/設計検討/修正まとめ統合計画.md §10.4 + §12` で正式予約済、SP-018 = Hermes/memory sprint との衝突回避のため SP-023 に変更)**: Quality Loop product artifact schema 実装、本 design doc を core spec として cross-reference
- **ADR-00026 候補 (P0.1、proposed 新規起票必須、SP-023 と対応する Quality Loop schema ADR、F-PR13-R3-003 P2 adopt: 本 ADR 番号は R29 plan §10.4 + §12 で Quality Loop schema 用に reserved、他 plan / 既存 ADR との衝突なし)**: Quality Loop schema design (DB / API / event_type)、本 design doc を background として cross-reference
