...
- **PII / secret-like value / raw tool args / raw realtime payload / client secret response body は role に関わらず常に redact (= 表示禁止)** — Codex PR #5 R2 F-PR5-005 P2 adopt:
  SP-009 acceptance criteria は role-based visibility を許可しない (raw tool args / raw realtime payload は完全 redact、redacted hash のみ表示)。
  archive 原文では「role-based visibility」と書いていたが、SP-009 invariant と矛盾するため本 adopt で「常に redact」に統一。
