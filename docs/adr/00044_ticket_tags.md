...
- `backend/app/repositories/tag.py` (新規): tag CRUD (create は name secret scan を内包する単一関数) +
  ...
  soft-delete + 削除 preflight (affected count)。全 query に `tenant_id` + project scope、active は deleted_at IS NULL。
- `backend/app/api/tags.py` (新規) or `tickets.py` 拡張: project-scoped endpoint (HIGH-1)。palette 検証 +
  active-project guard (HIGH-2) + assert_ticket_actionable。secret scan は repository 境界に委譲 (HIGH-3)。
...
- **soft-delete / restore (MEDIUM)**: tag 削除は deleted_at set のみで ticket_tags は保持。tag list /
  ticket tags embed から消えるが restore (deleted_at クリア) で復元。削除前に affected ticket 件数を
  preflight し `config_changed` audit に残す (raw secret なし)。
