...
- tag mutation の **capability / action class 境界** (`task_write` / read-only token 403、ADR Gate#4、R8)。
- tag delete + migration down (drop) の **rollback 方針** (ADR Gate#8)。delete は使用中不可で非破壊だが、
  schema rollback の drop を破壊的操作として gate trace に含める。
...
- **delete model (R3-R5 で収束、R9 で active-scope 精緻化)**: soft-delete + restore は同名再作成と衝突し
  rename/merge 解決契約が必要になり、hard-delete + 復旧 snapshot は audit durability 契約が必要になる。
  いずれも複雑なため採らず、**tag 削除は「active ticket での使用中なら 409、active 未使用なら hard delete」**
  に一本化する (下記 DELETE endpoint)。失う分類情報が構造的に存在しないため復旧 snapshot /
  affected_ticket_ids / soft-delete / restore のいずれも不要。audit は observability 用
  (`config_changed`: tag_id/name) のみで復旧用 snapshot ではない。使用中 tag は detach 後に削除。
  tag 名/色の誤りは rename (PATCH) で訂正できる。
- **active-scope 一貫性 (R7/R8/R9 HIGH)**: ticket は soft-delete (`tickets.deleted_at`、ADR-00037) され
  ticket_tags 行は生き残るため、**tag を扱う全 repository primitive は active ticket scope
  (`Ticket.deleted_at IS NULL` を join) を境界で enforce する**。`ticket_ids_with_tag` (filter) /
  `tags_for_tickets` (embed) は active ticket のみ返し、`delete_tag` の **使用中判定も active ticket への
  付与のみで数える** (deleted ticket への付与は in-use と見なさない)。これにより (a) 409 body の
  `attached_count` が active 件数のみを露出し hidden deleted ticket の付与数を推測させない (information
  leak 防止)、(b) 全付与が deleted ticket 側だけに残った tag が「active 0 件」でも永久に削除不能になる
  deadlock を防ぐ。
- **deleted attachment の lifecycle**: active 未使用 (active attachments = 0) の tag を削除する際、deleted
  ticket 側に残存する ticket_tags は **project lock (active-project guard の FOR UPDATE) 下で内部 cleanup
  してから** tag を hard delete する (FK2 RESTRICT を満たす)。**tag は「project label」として ticket とは
  独立に管理し、tag 削除後にその付与先 deleted ticket を restore してもその tag は復活しない** (tag は ticket
  metadata の補助であり、ticket 本体・タイトル・本文・status は restore で完全復活する)。この trade-off は
  「active で使われていない project label は即時整理できる」運用性を、「削除済 label の restore 復活保証」より
  優先した結果 (R9 案 A)。
...
- `DELETE /api/v1/projects/{project_id}/tags/{tag_id}` — tag delete。**active-project guard** +
  **使用中ガード: active ticket (deleted_at IS NULL) への付与が 1 件でも存在する場合は 409**
  (`{detail: "tag in use", attached_count}`、`attached_count` は active 付与数のみ — R9 information
  leak 防止)。active 未使用 (active attachments = 0) の tag のみ削除可能。これにより削除で失われる分類情報が
  構造的に存在せず、復旧 snapshot / cascade durability 契約が不要になる (Codex plan-review R4 HIGH 解決)。
  使用中 tag を消すには先に各 active ticket から detach する。**active 0 件のとき、deleted ticket 側に残存する
