...
- `DELETE /api/v1/projects/{project_id}/tags/{tag_id}` — tag delete。**active-project guard** +
  **使用中ガード: ticket_tags が 1 件でも存在する場合は 409** (`{detail: "tag in use", attached_count}`)。
  ...
  復旧用ではない)。FK2 の ON DELETE CASCADE は project 削除時の cleanup 用 (explicit tag delete は service
  で 0-attachments を `FOR UPDATE` で検査するため cascade で ticket_tags を一括消去しない)。
...
- `backend/app/repositories/tag.py` (新規): tag create (name secret scan を内包する単一関数) + rename /
  recolor + **delete (使用中 `FOR UPDATE` count > 0 なら 409、未使用のみ削除)** + ticket_tags attach/detach +
...
- **使用中 delete ガード (R1 MEDIUM / R3 / R4 HIGH)**: ticket_tags が付いた tag の DELETE → **409**
  (`attached_count` 提示)。0 attachments の tag のみ削除 200 + `config_changed` audit (tag_id/name、
  observability)。使用中削除を構造的に禁止するため復旧 snapshot / durability 契約は不要。detach 後に
  削除できることを確認。
