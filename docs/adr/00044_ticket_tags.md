...
- **delete model (R3 HIGH 解決)**: soft-delete + restore の連鎖は同名再作成と衝突し rename/merge の解決
  契約が必要になるため採用しない。tag 削除は **hard delete + 削除前 audit snapshot** とし、復旧は audit
  snapshot (affected_ticket_ids) からの recreate + reattach とする (下記 DELETE endpoint)。これにより
  restore 衝突を構造的に排除する。tag 名/色の誤りは rename (PATCH) で訂正できる。
...
- `DELETE /api/v1/projects/{project_id}/tags/{tag_id}` — tag **hard delete** (ON DELETE CASCADE で
  ticket_tags も削除)。**active-project guard** + **削除前 transaction 内 audit snapshot** (`config_changed`:
  actor / project / tag_id / name / color / **affected_ticket_ids** list + affected_count)。復旧は audit
  snapshot からの recreate + reattach (deterministic、手動)。soft-delete/restore を採らないため restore
  衝突は発生しない (R3 HIGH 解決)。
...
- tag 削除は **hard delete** (cascade) だが、削除前に transaction 内 audit snapshot (affected_ticket_ids)
  を残すため、誤削除しても recreate + reattach で deterministic に復旧可能 (Codex R1 MEDIUM / R3 HIGH 解決)。
  soft-delete を採らないため restore 衝突・rename 連鎖は発生しない。
- tag filter の N+1 / 大量 tag → per-ticket tags は単一 join query で bulk 取得 (ticket list で tag を
  個別 fetch しない)。

## rollback 手順

- **運用 rollback (誤削除)**: 削除時 audit snapshot の `affected_ticket_ids` から、同 name/color で tag を
  recreate し当該 ticket に reattach (手動だが deterministic)。
...
- `backend/app/repositories/tag.py` (新規): tag create (name secret scan を内包する単一関数) + rename /
  recolor + **hard delete (削除前 audit snapshot = affected_ticket_ids)** + ticket_tags attach/detach +
  per-ticket bulk tags + tag filter + active-project guard (`FOR UPDATE`)。全 query に `tenant_id` + project scope。
...
- `tests/api/test_ticket_tags.py` (新規): route 登録 + schema no-secret + SQL introspection
  ...
  **delete audit snapshot (affected_ticket_ids) + rename 同名衝突 409 + body project_id reject (R2/R3)**。
...
- **hard delete + audit snapshot (R1 MEDIUM / R3 HIGH)**: tag 削除は cascade で ticket_tags も消える一方、
  削除前 transaction 内 audit (`config_changed`) に `affected_ticket_ids` + count を残す (raw secret なし)。
  snapshot から recreate + reattach で復旧できることを確認。soft-delete/restore は持たないため restore
  衝突 test は不要。
