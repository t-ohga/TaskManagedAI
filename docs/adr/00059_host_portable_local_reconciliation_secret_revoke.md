...
- 実装対象ファイル:
  ...
  - `migrations/versions/00NN_secret_ref_material_lifecycle.py` (新規、material lifecycle tracking の additive 列: `material_state` (`writing`/`present`/`purging`/`purged`、create/rotate と revoke の crash-safe source of truth、ADR-00058 finding-2 と共有) + `material_purged_at` + `purge_attempts`。**downgrade は条件付き**: `status='revoked' AND material_purged_at IS NULL` 0 件 **かつ** `material_state IN ('writing','purging')` 0 件を preflight してから列削除、未収束残存時は fail-fast で source of truth を保護)
...
4. **material_lifecycle migration (0050) の downgrade (finding R8 反映)**: `material_state` / `material_purged_at` / `purge_attempts` 列削除は in-flight material の source of truth を消すため **無条件 lossless にしない**。downgrade は **(a) `status='revoked' AND material_purged_at IS NULL` が 0 件 かつ (b) `material_state IN ('writing','purging')` が 0 件** であることを **preflight で要求** (revoke 未 purge も create/rotate 途中も残っていないこと、未収束時は fail-fast し `secret gc-orphans` 実行を促す)。両条件 0 件確認後にのみ 3 列を削除。これにより revoke crash-window (revoked + material 残存) と create/rotate crash-window (writing 途中) の双方を downgrade で取りこぼさない。
5. 検証: restore preflight `verify_target_binding_consistency` が loopback で PASS、再登録した secret_ref の redeem 成功、crash-window test (revoke + create/rotate 双方) で reconciliation 収束、**downgrade preflight test (未 purge revoked or material_state writing/purging が残る状態で downgrade を fail-fast、gc-orphans 後に downgrade 成功)**。
