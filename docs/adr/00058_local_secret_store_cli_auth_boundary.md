...
- 実装対象ファイル:
  ...
  - `migrations/versions/00NN_secret_uri_local_backend.py` (新規、CHECK 拡張、両方向 + downgrade `secret://local/%` row 不在 preflight) — **Phase 0**
  - `config/cli_registry.toml` (modify、claude/codex は host-ambient 分類を明記、API key 経路のみ `credential_env_mapping`) — **Phase 0 (分類記載)**
  - `backend/app/services/secrets/broker_spawn.py` (新規、API key 用 **broker-owned spawn primitive** = broker 内部が resolve→inject→spawn→`assert_no_raw_secret`→zeroize を所有し result のみ返す。外部 callback は raw 非接触) — **Phase 2 (API transport 配線時)**
  - `backend/app/services/cli_artifact/launcher.py` (modify、host-ambient は HOME 経由自然参照。broker-owned spawn primitive から呼ばれる side は signature 不変) — **Phase 0 は host-ambient のみ、broker 注入は Phase 2**
...
- 実装ガイダンス:
  - **CredentialSupplyMode** を 2 経路に分離:
    ...
    - `broker_managed` (API key、副次、**Phase 2 実装** = API transport 配線時): static API key を secret_ref 登録 → broker-owned spawn primitive で注入。**self-rotating credential は broker_managed 経路へ登録できない validation を register 時に enforce** (案B の罠を構造防止)。
  - **【境界批評 finding-2 反映: broker-owned spawn primitive】**broker-managed API key の注入は、**callback が raw を受け取る方向を採らない** (既存 `redeem_capability_token` の operation callback は SecretHandle(metadata) のみ受領、raw を caller/callback へ返すと §10「値を返す API ではない」/ SecretHandle-only invariant に違反するため)。代わりに **broker 内部が所有する専用 spawn primitive** (`broker_spawn.py`) を新設: broker が atomic claim → 内部で raw resolve → spawn env へ inject → subprocess 起動 (cli_artifact launcher を broker が呼ぶ) → 結果に `assert_no_raw_secret` → raw を zeroize、までを **broker-owned code 内で完結**し、**外部 caller には CliResult のみ返す (raw は一切外へ出さない)**。任意の外部 callback に raw を渡す経路は作らない。launcher signature `launch_cli_agent(request, registry)` は不変 (agent allowlist bypass 防止)。**この primitive は Phase 2 (API transport) で実装**。Phase 0 は host-ambient のみのため本 primitive は設計確定のみ。
  - launcher の `assert_no_raw_secret(env)` (caller-influenced env を canary scan) は維持。broker-owned spawn primitive の注入分も result/log に対し canary scan を通す (bypass しない)。CLI 固有 token 形 (Claude accessToken / Codex refresh_token) を canary pattern 登録。
  - broker-owned spawn primitive が temp-file 経路を使う場合は `finally` + secure unlink で確実削除 (crash/SIGKILL/timeout kill 時の残留防止)。
  ...
  - URI 全体 regex (`secret://(sops|local)/<scope>/<name>#v<n>`、components_match も backend 可変) は単一定数 `SECRET_URI_PATTERN` に集約し migration / ORM / resolver dispatch / register validation / test EXPECTED が import (5+source drift 構造防止)。未知 backend は fail-closed deny。
- テスト指針:
  ...
  - `tests/secrets/test_broker_spawn_primitive.py` (**Phase 2**): broker-owned spawn primitive が result のみ返し caller/外部 callback に raw を返さない、result/log に `assert_no_raw_secret`、子 process 終了後残留なし、temp-file 経路は secure unlink、caller が credential を直接渡せない (signature 物理削除) negative。
  - migration 両方向 + downgrade preflight (`secret://local/%` row 不在検証)。
  - 実コード review: broker-owned spawn primitive (Phase 2) は §14.1 CRITICAL gate → codex-adversarial-review mode=code で findings_zero (CRITICAL=0/HIGH≤2)。Phase 0 の secret tooling (resolver/create/register/revoke) も CRITICAL invariant 直結のため同 gate。
...
| broker-managed 注入で raw が caller/env/argv に漏れる (Phase 2) | `assert_no_raw_secret`、test_broker_spawn_primitive negative | **broker-owned spawn primitive** が resolve→inject→spawn→scan→zeroize を内部所有し result のみ返す。外部 callback に raw を渡さない。signature から credential 引数削除 |
...
2. **CLI=host-ambient 分類**: revert は credential 供給を host worker 自然参照に戻すのみ (DB 変更なし、Phase 0 は元々 host-ambient のみ)。broker-owned spawn primitive (Phase 2) は additive (既存 redeem 経路不変) なので revert は primitive の未配線化。
