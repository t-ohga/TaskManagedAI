...
> **status: accepted (2026-06-20)**。Phase 0 詳細設計 workflow (4ストリーム並列設計→境界批評→最終化) の成果。**codex adversarial plan-review R1-R20 で計 33 HIGH + 1 MEDIUM を全 adopt、R20 で approve / SHIP-READY (No material findings) を達成**し、user 承認済の 4 決定 (案C hybrid / host worker / loopback / keyring) + §12.4 gate (codex-plan-review clean + 採否判定) をもって accepted 昇格。境界批評が初期設計を **REFUTE** (broker が resolve した raw を launcher へ forward する seam は SecretBroker §10「値を返す API ではない」を破る) し、修正版 (CLI=host-ambient で broker 非経由、API key=broker 内 in-process `provider.call` で subprocess/env/argv/temp-file に raw を出さない) に到達。adversarial R1-R7 で計 12 HIGH + 1 MEDIUM を adopt。実装着手直前に codex-plan-review clean + 採否判定 を経て accepted へ昇格 (sprint-pack-adr-gate §12.4)。
...
- 実装ガイダンス:
  ...
  - **【境界批評 R7 finding-2 反映: create/rotate material lifecycle + identity】** material は `tenant_id + secret_ref_id` に束縛 (cross-tenant 同名衝突防止)。create/rotate の **crash-safe 順序**: ① secret_refs row を `pending` + `material_state='writing'` で INSERT/commit (DB が material owner の source of truth) → ② store へ raw 書込 (key=tenant_id+secret_ref_id) → ③ row を `material_state='present'` + (create 時) `active` 昇格 commit。途中 crash / store 失敗 / DB 失敗を `material_state` (`writing`/`present`/`purging`/`purged`) + reconciliation で収束: `material_state='writing'` のまま放置された row は `secret gc-orphans` が store の有無を確認し再試行 or rollback (orphan material 削除 + row 破棄)。idempotency key で再実行を冪等化。revoke 側の `material_purged_at` と対称な create 側 durable tracking。**DB default は `material_state='writing'`** (省略 insert が false-present にならない安全側)、migration 0050 は既存 active/deprecated/**pending (material を持つ、rotation 中 verify 対象)** 行を `present` へ backfill、revoked 行は material_purged_at NULL のまま gc-orphans で reconcile。**token issue / redeem の secret_ref 再検証で `material_state='present'` を必須化** (`writing`/`purging`/`purged` は `material_not_present` で deny、boundary §7/§9)。これにより store 未完了の false-present row から token が出ない。
  - **keyring 依存** (user 承認済): macOS Keychain backend。**local Mac (Phase 0 実運用 target) では keyring は must_ship** (Keychain が OS-level に master/material を保護)。cryptography Fernet 暗号化ファイル fallback は **CI Linux headless / Keychain 不在環境専用**。supply-chain audit 対象として uv.lock pin + SBOM 対象化 (**ADR-00020 Framework Intake Checklist** = license/attribution/SBOM/secret canary の dependency intake gate、`scripts/ci/check_framework_intake.sh`)。
...
| CLI が token を stdout/stderr/output artifact に echo → launcher capture に raw 混入 | output 固有 credential canary scan (`credential_canary.py`) + 検出時 `CREDENTIAL_EXFILTRATION` deny | launcher が capture 後に canary scan (JWT / `codex_refresh_token` (`rt.N.<base64url>`) / anthropic OAuth / **JSON key-name canary** / credential-file path basename + broad scanner)、hit で `CREDENTIAL_EXFILTRATION` Hard Gate failure。**orchestrator は CREDENTIAL_EXFILTRATION path で raw artifact を再 redact せず `[withheld: credential_exfiltration]` placeholder のみ emit** (narrower `_RAW_SECRET_PATTERNS` で raw credential が survive する経路を物理削除、AC-HARD-02)。残る原理制約は §残リスク参照 |
...
| keyring 新規依存の supply-chain | uv.lock pin + SBOM + ADR-00020 intake | **ADR-00020 Framework Intake Checklist** で intake (license/attribution/SBOM)。暗号化ファイル fallback で degrade 可 (cryptography 既存) |
...
### 残リスク (gate C credential exfiltration、honest 記載)

**全 drain path が fail-closed (deny + cancel 両方)**: launcher の post-completion canary scan は normal-completion path のみを保護する (`CredentialCanaryHit` 検出時 `CREDENTIAL_EXFILTRATION` raise → orchestrator が `_withheld_redaction()` placeholder を返す)。**cancel-during-launch path は launcher scan より前に `CancelledError` が raise されるため launcher scan を通らない**が、orchestrator の `_drain_artifact_streams` 自体に credential canary scan を組み込み、hit 時は narrow `redact_stream` を呼ばず withheld placeholder を返す uniform defense-in-depth を実装済 (Codex adversarial HIGH sibling-path fix)。これにより **deny path / cancel-during-launch path / cancelled-before-launch path の全 drain 経路が canary hit 時 withhold で fail-closed** となり、cancel race 中に malicious CLI が credential を `--output-last-message` へ echo しても raw token が `redacted_text`/`content_hash` に残り artifact/AgentRunEvent へ永続化される経路は無い。

**scan と redaction の正規化統一 (normalization-mismatch fix)**: canary scan (`scan_for_credential_exfiltration`) と `redact_stream` は **同一の `normalize_for_scan`** (ANSI escape / 残留 `\x1b` / C0・C1 control / Unicode Cc・Cf carpet-bomb を strip、tab/LF/CR のみ保持) を **照合前に適用**する。これにより、攻撃者が credential token に不可視文字 (U+200B ZWSP / ANSI `\x1b[0m` / C1 `\x85` / BOM 等) を注入して raw-text scan を擦り抜け、redaction が同 char を strip して token を再構成する **obfuscation bypass のクラスを構造的に閉じる** (scan と redaction が同一正規化テキストを見るため divergence しない)。さらに **credential 固有 5 pattern (JWT / `codex_refresh_token` / anthropic OAuth / key-name echo / path echo) を `redact_stream` の backstop に fold** し (shared `_RAW_SECRET_PATTERNS`/`_SECRET_TEXT_PATTERNS` の exact-set drift guard は不変、credential_canary 側 pattern を lazy import で併用)、scan が万一漏らしても redaction が必ず redact する二重 fail-closed とした。以下は **それでも原理的に value/structure-scan では閉じ切れない残リスク** として明記する (accepted HIGH risk、local 単一 user・AI は user 自身の subscription という前提で許容):

1. **任意 re-encode された token blob (HIGH→残)**: prompt-injected な CLI が credential token 値を base64 / hex / 任意 transform で再 encode し、**かつ周囲の JSON key 構造 (`"refresh_token":` 等) も削った形**で出力すると、value-pattern も key-name canary も hit しない。**不可視文字 (ZWSP/ANSI/C1) による obfuscation は上記 normalization 統一で閉じた**。**素の credential file dump / 改行分断 JWT は JSON key-name canary が encode 非依存で捕捉する**。残るのは「token 値を可逆変換しつつ key 構造ごと潰す」任意 transform のみで、これは出力 scan の本質的限界であり、防御は **control 2 の per-agent 最小 HOME (他 secret 非同居で blast radius を構造的に最小化) + Phase 2 の追加封じ込め (tool-level read deny / sandbox)** に依存する。
2. **任意 file 名の credential (LOW→残)**: path-echo canary は `auth.json` / `*.credentials.json` / `id_rsa` 等の **既知 basename** を捕捉するが、custom 命名の credential file は対象外。per-agent 最小 HOME に既知 file のみ置く運用で緩和するが、任意 file 名は scan 範囲外。
3. **Phase 2 integration test scope**: 上記 1-2 を含む「実 codex を任意 (malicious) prompt で動かし、実 credential が stdout/artifact へ流れないことを end-to-end 確認する integration test」は real codex binary + 実 credential を要し CI 不可のため、**大元計画 Phase 2 (CLIAgentAdapter 本体) の integration test へ narrow defer**。Phase 0 は mock output に対する canary scan の unit test + per-agent HOME env 構築 test + orchestrator withhold test (deny path + cancel-during-launch path 両方) で control を検証する。Phase 2 では tool-level read deny / sandbox による「そもそも credential file を read させない」封じ込めを追加し、出力 scan を最終防御層に格下げする。

これらの残リスクが confidential 以上の payload を扱う自律実行を **Phase 2+ の追加封じ込めまで gate** する根拠 (上表「confidential 以上 payload の自律実行は Phase 2+ 追加封じ込めまで gate」と一貫)。
