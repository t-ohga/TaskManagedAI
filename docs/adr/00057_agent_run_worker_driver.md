...
>
> R1+R2 により本 slice は **当初想定 (queued run を step に流すだけ) より substantial な core-runtime piece**: atomic claim + cancel entrypoint 統一 + state machine additive 拡張 (queued→failed / transient→cancelled) + enqueue fail-closed + provenance binding + non-terminal closure 分離 + 実装後 full adversarial loop。実装着手直前に plan-review R3 で R2 fix の副作用確認 → clean → §sprint-pack-adr-gate.md §12 に従い proposed → accepted 昇格 + 採否判定経由。
...
- **task** (`backend/app/workers/agent_run_driver.py` 新規): `async def execute_agent_run(ctx, *, run_id, tenant_id)`:
  ...
  5. **non-terminal outcome handling (plan-review F5 + R2-F3、must_ship、closure policy を分離)**: `provider_incomplete` と `validation_failed` は **state machine 上の許可遷移が異なる**ため closure を分ける (R2-F3、実コード `state_machine.py:35-36` 照合済):
     - `validation_failed` → `execute_repair_decision_step` で bounded retry → **`repair_exhausted` terminal** (許可遷移 `validation_failed -> repair_exhausted`)。
     - `provider_incomplete` → bounded retry は **`running`** (`repair_retry_scheduled`)、retry 不能 / 上限到達は **`failed`** (`run_failed`) (許可遷移 `provider_incomplete -> {running, failed, cancelled}`、**`provider_incomplete -> repair_exhausted` は不許可**)。`repair_exhausted` へ送らない。
     - Mock を forced incomplete / validation_failed / refusal / unsupported_schema / schema_mismatch にする negative test を **各 closure path 個別**に含める (happy path だけにしない)。
  ...
  7. **cancellation (plan-review F1 + R2-F2、must_ship、全 entrypoint 統一 + DB-status 検知)**: 実コード照合で **`cancel_agent_run` (cancel.py) は DB-first** (run を `transition_with_event` で `cancelled` に遷移 commit してから best-effort で `publish_cancel_signal` を per-run channel `cancel:run:{run_id}` に publish) と判明。よって driver の cooperative cancel は **Redis transport でなく DB status 再読**を正本にする:
     - **driver は step 境界で run.status を refresh** → `cancelled` (cancel_agent_run / MCP cancel が DB に set) を検知したら graceful stop (driver は cancelled へ自分で遷移しない、cancel 経路が既に記録済)。transient state 中に cancel が race して次 transition が validation fail したら catch → status 再読 → cancelled なら no-op stop (F3 benign)。
     - **全 cancel entrypoint 統一 (R2-F2、実コード `api_bridge.py:653` 照合済)**: MCP `bridge_run_cancel` は現状 `run.status="cancelled"` を直接 set + commit し **`run_cancelled` event を append しない** (AgentRunEvent 正本 invariant 破壊) + signal も飛ばさない。これを `cancel_agent_run` service 経由に統一 (event append + signal)。`bridge_run_cancel` / `server.py:run_cancel` に actor_id を plumbing。同様の直接 UPDATE (`api_bridge.py:1686` の bulk/superintendent cancel) も event-append 経路へ寄せるか別 follow-up に切り出す (本 ADR §残課題)。
...
- **state machine additive 拡張 (R2-F1 + R2-F2 由来、CRITICAL invariant、Python+test のみ、migration 不要)**: `ALLOWED_TRANSITIONS` + `EVENT_TYPE_FOR_TRANSITION` に `queued->failed` (run_failed) + `{queued, gathering_context, generated_artifact, schema_validated}->cancelled` (run_cancelled) を additive 追加。16 status enum / event exact set は不変 (新 status / 新 event_type なし)。transition は DB CHECK 対象外 (status enum のみ DB CHECK)、5+ source 整合は state_machine.py + 該当 test の `EXPECTED_*` で固定。
...
- **cancellation (F1 + R2-F2)**: cooperative cancel は **DB status 再読**が正本 (cancel_agent_run は DB-first)。provider mid-call kill は不可 (step 境界のみ) → job_timeout (300s) + 後続で kill policy。Redis transport (subscribe vs per-run channel) 不整合は mid-call interruption 用で本 slice 対象外。全 cancel entrypoint (REST + MCP) を `cancel_agent_run` へ統一し `run_cancelled` event を必ず append、driver は DB status で stop。実 cancel_agent_run + MCP run_cancel 両経路で integration test。
...
- **non-terminal outcome (F5 + R2-F3)**: closure を **state machine 許可遷移に合わせて分離**。`validation_failed` → `execute_repair_decision_step` → `repair_exhausted` terminal。`provider_incomplete` → bounded retry は `running`、上限/retry 不能は `failed` (`provider_incomplete -> repair_exhausted` は不許可)。Mock forced incomplete / validation_failed / refusal / unsupported_schema / schema_mismatch を各 closure path 個別 negative test。
...
- `backend/app/services/agent_runtime/state_machine.py` (additive: `ALLOWED_TRANSITIONS` + `EVENT_TYPE_FOR_TRANSITION` に `queued->failed` / `{queued,gathering_context,generated_artifact,schema_validated}->cancelled`、16 status / event exact set 不変)
...
- **non-terminal handling (F5 + R2-F3、closure 分離)**: Mock forced `validation_failed` → driver repair loop → `repair_exhausted` terminal。Mock forced `provider_incomplete` → bounded retry `running` → 上限で `failed` (**`repair_exhausted` に行かないことを assert**)。refusal → `provider_refused` terminal。unsupported_schema / schema_mismatch も各 close を検証 (happy path 以外を必ず close)。
- **cancellation (F1 + R2-F2、entrypoint parity)**: (a) REST `cancel_agent_run(run_id)` で run を cancelled に → driver が step 境界 DB-status 再読で検知し graceful stop。(b) MCP `run_cancel` でも同様に stop **かつ `run_cancelled` event が append される** (統一前は event 非 append だったため両経路で event 検証)。(c) queued / transient state からの cancel が allowed (新 edge) で `run_cancelled` event 付きで cancelled 到達。
