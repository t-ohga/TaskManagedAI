...
>
> R1+R2+R3 により本 slice は **当初想定より深い core-runtime piece** (durable state machine / commit semantics / crash recovery の hardness)。foundational scope は最小 robust に確定: atomic claim + happy-path 駆動 + cancel (DB-status + entrypoint 統一 + 明示 commit) + enqueue fail-closed + provenance + non-terminal closure (validation_failed→repair_exhausted / provider_incomplete→failed) + state machine additive edge。**durability hardening (durable retry counter / crash resume / reclaim sweeper) と production runtime は明示 defer**。実装着手直前に plan-review R4 で R3 fix の副作用確認 → clean → §sprint-pack-adr-gate.md §12 で proposed → accepted 昇格 + 採否判定。
