...
- **cancellation (F1 + R2-F2 + R3-F1 + R7-A2)**: cooperative cancel は **post-commit DB status 再読**が正本 (`cancel_agent_run` は commit しない — caller が commit)。`bridge_run_cancel` を `cancel_agent_run` 経由に統一する際は **明示 commit を残す** (落とすと MCP session yield 後 rollback)。**R7-A2: `cancel_agent_run` は commit 前に Redis publish を await するため、Redis black-hole 化で cancelled 遷移が未 commit 固着しうる。MCP cancel は signal 非依存 (driver は DB-status で検知) なので no-op publisher を渡し DB-first commit にする** (Redis 依存を外す)。provider mid-call kill は不可 (step 境界のみ) → job_timeout (300s) + 後続で kill policy。全 cancel entrypoint (REST + MCP) で `run_cancelled` event append + 実 DB 永続を integration test。REST 経路の pre-commit publish hang (cancel_agent_run 共有) は別 follow-up。
...
- **worker-crash-mid-drive / 補償失敗後の resume / reclaim (R3-F2 + R7-A1 派生、durability)**: atomic claim は `status='queued'` のみ対象のため、driver が transient state (gathering_context / running / generated_artifact / schema_validated) まで進めた後 (a) worker crash、または (b) **R7-A1: 例外後の fail-closed 補償自体が DB/event 障害で失敗** すると、run が非終端のまま残り arq retry は claim-miss=no-op (再駆動しない)。**R6-A2 の re-raise は observability (arq に例外 surface、silent failed-success を出さない) を提供するが auto-recovery ではない**。stale/stuck-run の resume/reclaim には **claim owner / attempt / lease の永続化** が要り、worker-crash と同一の **deferred crash-recovery class** として後続 increment へ defer。本 slice の保証は「double-drive しない (atomic claim) + silent loss を作らない (stuck は status で可視) + 補償失敗を surface する (R6-A2)」までで、「crash / 補償失敗後の自動 re-drive」は scope 外 (lease/sweeper)。
