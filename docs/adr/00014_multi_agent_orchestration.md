...
## SecretBroker Multi-Agent Deny Reasons (PE-F-014)

Multi-agent 文脈での SecretBroker deny reason codes (6 種):

| reason_code | 条件 |
|---|---|
| `agent_decider_forbidden` | orchestrator / agent が approval decider を試行 |
| `tier_2_agent_decider_attempt` | Tier 2 auto-allow profile から approval decider 経路へ escape |
| `actor_type_mismatch` | expected actor_type と DB 上の Actor.actor_type が不一致 |
| `role_id_mismatch` | expected role_id と DB 上の AgentRun.role_id が不一致 |
| `lease_expired_no_secret_access` | lease expired 状態で secret access |
| `progress_lease_violated` | progress lease 違反 run の secret access |
