...
1. advisory lock 下で latch check (engaged なら spawn 中止)。
2. `managed_agents` 行を **pre-register (state=`spawning`、latch generation 刻印)** で commit。
3. process 起動 (`create_subprocess_exec`、`start_new_session=True`)。
4. pid / process_group_id を確定し行更新 (state=`running`)。
5. **compensating path**: 起動失敗・commit 失敗・crash 時は pre-register 行を terminal (state=`failed`/`stopped`) 化。spawn 完了直前に latch generation を再確認し、engage 割込みなら self-abort (process kill + 行 terminal)。

supervisor は kill 時 **state∈{spawning, running}** を全対象 (pid 未確定の spawning 行は「起動完了待ち→kill」or「spawn 中断」)。managed_agents の `state` enum: `spawning`/`running`/`stopped`/`failed`。
...
- **honest limit**: 同期 `provider.execute` は engage で中断不能。CAS は engage 後の usage 記録 / artifact 永続化 / status 進行を防ぐが、**engage 時点で既に in-flight な単一 call の provider 側課金確定は防げない** (その 1 call 分のコストは発生し得る)。新規 provider call は同期 latch check で deny されるため被害は engage 時の in-flight 1 call に限定。本限界を §残リスクに明記。
