...
| SP-009-5 P0.1 UI deferred surfaces | `partial_skeleton` | Batch A `/today`, Batch B `/timeline`, Batch C decision packet hash visibility, and Batch D1 notification triage DB/API contract are implemented; D2 notification UI, Newcomer Path, and `request_revision` remain |
...
- SP-009-5 continuation: implement notification triage D2 UI/actions on top of the D1 API contract; `request_revision` still requires ADR/API/state-machine planning; Newcomer Path is P0.1 polish.
