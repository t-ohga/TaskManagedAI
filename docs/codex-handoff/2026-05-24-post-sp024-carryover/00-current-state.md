...
- Remote `main` was verified at `517ab4433d60aa2d75cec487a68250ad8a14049d` after PR #239.
- Open GitHub PR list was empty after PR #239 merge.
...
| SP-009-5 P0.1 UI deferred surfaces | `partial_skeleton` | Batch A `/today`, Batch B `/timeline`, Batch C decision packet hash visibility, Batch D1 notification triage DB/API contract, Batch D2 notification UI/actions, Batch E0 `request_revision` contract plan, Batch E1 DB/API, Batch E2 Approval Detail UI, Batch E3 revised artifact handoff, and Batch F0 Newcomer Path contract plan are implemented; F1+ Newcomer Path implementation remains |
...
- SP-009-5 continuation: implement Newcomer Path F1 read-only `/onboarding` as P0.1 polish, using the F0 contract plan before any API/schema/runtime work.
