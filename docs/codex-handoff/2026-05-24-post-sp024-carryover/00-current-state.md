...
| SP-009-5 P0.1 UI deferred surfaces | `partial_skeleton` | Batch A `/today` and Batch B `/timeline` read-only UIs are implemented with existing APIs; decision packet hash visibility, notification triage, Newcomer Path, and `request_revision` remain |
...
- SP-009-5 read-only UI continuation: decision packet hash visibility. Notification triage and `request_revision` require ADR/API contract gates.
