...
4. For SP-009-5 Batch F, use `plans/task-16-sp0095-newcomer-path-contract-plan.md`; F1 read-only `/onboarding`, F2a planning, F2b response-only backend API, F3 dry-run plan-review UI, and F4 CLI onboarding parity are implemented. Next is F5 closeout.
