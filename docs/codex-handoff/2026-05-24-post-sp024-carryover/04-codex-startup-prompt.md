...
Start with Newcomer Path F5 closeout if the user asks to continue SP-009-5 Newcomer Path. Otherwise start with task-01 unless the user explicitly selects another task.
...
- For SP-009-5 Newcomer Path, read `plans/task-16-sp0095-newcomer-path-contract-plan.md` and `plans/task-18-sp0095-newcomer-path-f2-contract-plan.md` first. F1 read-only `/onboarding`, F2a planning, F2b response-only backend API/schema work, F3 dry-run result rendering, and F4 CLI onboarding parity are implemented; F5 must close docs/status/verification drift without adding new runtime shortcuts.
