...
Start with Newcomer Path F2 planning if the user asks to continue SP-009-5 Newcomer Path. Otherwise start with task-01 unless the user explicitly selects another task.
...
- For SP-009-5 Newcomer Path, read `plans/task-16-sp0095-newcomer-path-contract-plan.md` first. F1 read-only `/onboarding` is implemented; F2+ API/schema/runtime work needs a separate contract gate.
