...
| 2 | `taskhub_active_registry_fleet_membership_unavailable` | active_registry_fleet.signed.json 不在 (`<config_dir>/active_registry/`) | - | fleet deploy 実行 |
...
#   重要: `--signed-journal` は skeleton flags (`--integrity` / `--network-invariant` /
#   `--multi-agent`) と mutually exclusive (`_cmd_verify` で return 2)。**別 run で実行**。
#   さらに `--signed-journal` は **audit event signed journal** (id / event_type / tenant_id /
#   event_payload / created_at schema) を verify するため、approval issuance journal
#   (issuance_journal.signed.jsonl) は対象外 (schema 違い、verify fail する)。
#   approval issuance journal の verify は scripts/taskhub_approval_issuance.py の
#   verify_issuance_chain_invariants() を `PYTHONPATH` 経由で直接 invoke する。
#   時間範囲 filter は SP-013 で実装予定。
for HOST in vps linux mac; do
  # 1) integrity + network-invariant verify (skeleton mode、signed-journal 排他)
  ssh "$HOST" 'taskhub verify --integrity --network-invariant'

  # 2) signed-journal verify (audit event signed journal、`<config_dir>/audit_events/*.signed.jsonl`)
  # SP-012 期間中 audit_events directory の実装が batch4 で integration 予定、
  # SP-013 で本格運用化。SP-012 timeframe では本 step は spec として記述。
  # ssh "$HOST" 'taskhub verify --signed-journal --input /etc/taskhub/audit_events/recent.signed.jsonl'

  # 3) approval issuance journal verify (chain hash + monotonic clock + key status)
  ssh "$HOST" "PYTHONPATH=/opt/taskhub/scripts:/opt/taskhub python3 -" <<'PYEOF'
import sys, json
from pathlib import Path
from taskhub_approval_issuance import verify_issuance_chain_invariants

journal_path = Path("/etc/taskhub/approvals/issuance_journal.signed.jsonl")
entries = []
if journal_path.exists():
    for line in journal_path.read_text().splitlines():
        if line.strip():
            entries.append(json.loads(line))
ok, reason = verify_issuance_chain_invariants(entries)
print(f"issuance_journal verify: ok={ok} reason={reason}")
sys.exit(0 if ok else 1)
PYEOF
done
```
...
> **CLI 状況**: Mode B (TPM monotonic counter + signed attestation) は **SP-013/SP-014 で実装予定** の future spec。SP-012 では Mode A (Linux CLOCK_MONOTONIC) 固定。本 §21.2 は spec として記述、TPM mode を有効化する operator は **本番運用前に tpm2-tools の実コマンド + output schema を verify** すること。

```bash
# TPM monotonic counter は TPM2 clock 経由で取得 (tpm2-tools v5+ 想定):
#   tpm2_readclock は YAML 出力 (NOT JSON)。time / clock / safe / reset_count を持つ。
#   monotonic 値として安全な field は `clock` (TPM 起動以降の milliseconds、reset_count で attest)。
sudo tpm2_readclock
# 出力例 (YAML):
#   time: 12345
#   clock: 12345
...
#   reset_count: 0
#   restart_count: 0
# monotonic value 抽出 (yq 経由):
sudo tpm2_readclock | yq '.clock'  # milliseconds since TPM init (reset_count で attest 必要)

# signed attestation (TPM2 quote 経由、本 PR では skeleton spec):
#   sudo tpm2_quote -c <PRIMARY> -l sha256:0,1,2,3 -m quote.msg -s quote.sig
#   operator が signed attestation を keyring_state.head.signed に embed する
#   (SP-013 で `TASKHUB_MONOTONIC_CLOCK_MODE=tpm_clock` env + helper module で automation 化)
```
