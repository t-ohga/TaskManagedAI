...
print(f'key (taskhub1...): taskhub1{base64.b64encode(pub).decode()}')
print(f'private (DO NOT LOG): {priv.private_bytes_raw().hex()}')
"
...
for HOST in vps linux mac; do
  ssh "$HOST" 'python3 -m scripts.taskhub_keyring verify \
    /etc/taskhub/approval-verify-keyring.signed.json \
    /etc/taskhub/approval_keyring_root.pub'
done
```
...
ssh "$SOURCE_HOST" "taskhub thaw --reason 'cutover rollback'"

# Scenario B: cleanup (source freeze 解除、target 何もしない)
ssh "$SOURCE_HOST" "taskhub thaw --reason 'cutover aborted before phase alpha'"
...
> **CLI 状況**: `taskhub active-registry` は SP-012 では skeleton (subcommand なし、help のみ)。verify は `python3 -m scripts.taskhub_active_registry_gate` で直接実行する。

```bash
# active marker chain 検証 (scripts/taskhub_active_registry_gate.py の evaluate_gate を直接 invoke)
for HOST in vps linux mac; do
  ssh "$HOST" 'python3 -c "
import json
from pathlib import Path
m = Path(\"/etc/taskhub/active_registry/active.signed\")
if m.exists():
    print(json.dumps({\"host\": \"$HOST\", \"active\": True, \"data\": json.loads(m.read_text())}))
else:
    print(json.dumps({\"host\": \"$HOST\", \"active\": False}))
"'
done
...
1. **全 host で個別 freeze を実行** (`--all-hosts` flag は未実装、operator が host loop で順次):
   ```bash
   for HOST in vps linux mac; do
     ssh "$HOST" "taskhub freeze --reason 'split-brain detected'"
   done
   ```
...
5. fleet membership 再 sign + deploy + 各 host で再 verify (`python3 -m scripts.taskhub_active_registry_gate` で)
...
for HOST in vps linux mac; do
  ssh "$HOST" "taskhub freeze --reason 'incident-<ticket-id>'"
done
...
jq 'select(.reason_code | startswith("taskhub_signed_approval_keyring"))' \
...
# step 4: audit event の verify (`taskhub verify` subcommand は SP-012 で実装済)
for HOST in vps linux mac; do
  ssh "$HOST" "taskhub verify --since '<revocation_time>'"
done
```
...
- name: deploy approval keyring
  run: |
    ...
    for HOST in $(jq -r '.hosts[] | select(.status == "active") | .host_id' \
                  /etc/taskhub/active_registry_fleet.signed.json); do
...
#   python3 -m scripts.taskhub_keyring で KeyringStateHead を構築、operator が
#   approval_keyring_root の私 key で sign + atomic install。
python3 -c "
from scripts.taskhub_keyring import KeyringStateHead
import json
