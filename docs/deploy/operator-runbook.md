...
import sys, json
from pathlib import Path
from scripts.taskhub_keyring import verify_signed_manifest, SignedKeyringManifest
...
```bash
# active marker chain 検証 (scripts/taskhub_active_registry_gate.py の helper を直接 invoke)
for HOST in vps linux mac; do
  # 重要: local $HOST を ssh remote 内で評価するため double-quote で渡す
  ssh "$HOST" "python3 -c '
import json
from pathlib import Path
m = Path(\"/etc/taskhub/active_registry/active.signed\")
host = \"$HOST\"  # local loop var (ssh の outer double-quote で展開済)
if m.exists():
...
#   下記 `python3 -c` で KeyringStateHead を構築 (scripts.taskhub_keyring の dataclass を import)、
