...
| SP0095-UX-01 | Newcomer Path first-use flow | 9 → 9.5 → partial | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch F0 で contract plan を固定。次は F1 read-only `/onboarding` route。F2+ guided intake dry-run API/schema/runtime は別 gate | first-run mutating AgentRun 禁止、`tm` canonical、no raw secret/log/payload exposure |
