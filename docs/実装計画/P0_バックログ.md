...
| SP0095-API-02 | Approval `request_revision` loop | 9 → 9.5 → partial | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch E0 で contract plan を固定し、Batch E1 で additive `approval_revision_requests` table、`POST /api/v1/approvals/{approval_id}/request_revision`、human-only guard、rationale raw-secret scan、metadata-only audit/notification を追加済。E2 UI と E3 revised-artifact handoff は未実装 | ADR-00003 / ADR-00004 / ADR-00009 gate + migration up/down |
