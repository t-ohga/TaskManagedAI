...
| BL ID | 内容 | 元 Sprint | 残作業 | 検証 |
|---|---|---|---|---|
| BL-0103a | GET /api/v1/tickets list + detail route 実装 + tenant boundary | 9 → 11 | FastAPI route + repository contract + 越境 negative test | `tests/api/test_tickets_route.py` |
| BL-0106a | GET /api/v1/agent_runs list + detail route 実装 | 9 → 11 | 既存 POST /cancel に加えて list / detail | `tests/api/test_agent_runs_list.py` |
| BL-0107a | GET /api/v1/audit_events route + cursor pagination + tenant filter | 9 → 11 | FastAPI route + repository contract | `tests/api/test_audit_events_route.py` |
| BL-0107b | RedactedAuditPayloadSchema / RedactedAgentRunEventPayloadSchema (frontend) | 9 → 11 | backend `_payload_secret_scan.py` を frontend に port + DOM secret scan test | `tests/contracts/test_ac_hard_02_frontend_redaction.py` |
| BL-EnumDrift | TicketStatus / AgentRunStatus / AgentRunEventType / AuditEventType / PayloadDataClass の cross-source drift contract test | 9 → 11 | backend Literal / DB CHECK / frontend Zod の exact set 比較 | `tests/contracts/test_frontend_backend_enum_drift.py` 新規 |
