...
| SP0095-API-02 | Approval `request_revision` loop | 9 → 9.5 → partial | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch E0 で contract plan を固定済。初期実装では `revision_requested` status を追加せず、old approval は `invalidated`、`approval_revision_requests` が human rationale / snapshot / supersession を保持し、revised artifact が new approval row を作る。E1 DB/API、E2 UI、E3 revised-artifact handoff は未実装 | ADR-00003 / ADR-00004 / ADR-00009 gate + migration up/down |
