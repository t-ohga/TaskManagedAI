...
| BL-0107b | RedactedAuditPayloadSchema / RedactedAgentRunEventPayloadSchema (frontend) | 9 → 11 → partial | 2026-05-24 contract cleanup で frontend API schema が raw payload/value field を reject し、agent_runs/audit unit tests を追加済。残作業は browser DOM secret scan regression と rendered payload non-exposure の E2E 確認 | `frontend/__tests__/lib/api/agent-runs.test.ts` + `frontend/__tests__/lib/api/audit.test.ts` + DOM secret scan test 新規 |
| BL-EnumDrift | TicketStatus / AgentRunStatus / AgentRunEventType / AuditEventType / PayloadDataClass の cross-source drift contract test | 9 → 11 → partial | 2026-05-24 contract cleanup で TicketStatus / TicketPriority / AgentRunStatus / BlockedReason / AgentRunEventType の backend/frontend exact-set contract と AuditEventType open-string guard を追加済。残作業は PayloadDataClass UI contract と、backend AuditEventType registry を導入する場合の exact-set 昇格 | `tests/contracts/test_frontend_backend_enum_drift.py` |

### SP-009-5 carry-over (P0.1 UI deferred surfaces)

| ID | 内容 | 元 Sprint | Pack | 残作業 | Gate |
|---|---|---|---|---|---|
| SP0095-UI-01 | Today/Inbox control plane + minimal KPI strip read-only UI | 9 → 9.5 → partial | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch A で `/today` route、navigation、read-only lanes、KPI strip、Vitest/browser smoke を追加済。due_at は既存 API にないため due 表示は未実装 | no mutation |
| SP0095-UI-02 | Unified execution timeline read-only UI | 9 → 9.5 → partial | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch B で `/timeline` route、AgentRunEvent / AuditEvent / pending Approval / KPI summary の統合表示を追加済。Budget は専用 API ではなく既存 event types 経由のみ | raw payload DOM exposure 禁止 |
| SP0095-UI-03 | Decision packet hash visibility | 9 → 9.5 → completed | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch C で `/approvals/[id]` Decision packet panel と `stale_after_event_seq` additive API field を追加済。hash-like field は SHA-256 形式のみ DOM 表示 | no mutation |
| SP0095-API-01 | Notification triage minimal lifecycle | 9 → 9.5 → completed | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch D1 で required_action / severity / due_at / snooze / dedupe / resolved fields、redacted triage endpoint、actor-owned snooze/resolve、frontend API schema、migration up/down 検証を追加済。Batch D2 で `/notifications` triage UI/actions、state tabs、server-owned snooze action、resolve action を追加済 | ADR-00003 event schema gate |
| SP0095-API-02 | Approval `request_revision` loop | 9 → 9.5 → completed | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch E0 で contract plan、Batch E1 で additive `approval_revision_requests` table/API/service、Batch E2 で Approval Detail UI/server action、Batch E3 で revised-artifact handoff service と fresh decision-packet negative tests を追加済。AgentRun resume automation と public replacement approval API は後続 runtime batch | ADR-00003 / ADR-00004 / ADR-00009 gate + migration up/down |
| SP0095-UX-01 | Newcomer Path first-use flow | 9 → 9.5 → completed | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch F0 で contract plan を固定し、Batch F1 で read-only `/onboarding` route、Batch F2a で response-only dry-run intake API contract、Batch F2b で `/api/v1/onboarding/dry_run_plan` backend API、Batch F3 で dry-run plan-review UI、2026-05-25 Batch F4 で `tm` canonical CLI onboarding parity、Batch F5 で route/API/CLI parity closeout と no-persistence decision を同期済 | first-run mutating AgentRun 禁止、`tm` canonical、no raw secret/log/payload exposure |
...
| BL ID | 内容 | 元 Sprint | 残作業 |
|---|---|---|---|
| BL-0082 | repo 外 trusted wrapper (`~/.claude-trusted/taskmanagedai-hook-wrapper.sh`) | 7 → Phase 5 | 2026-05-24 plan/helper PR で wrapper install sequence と read-only verifier を追加済。残作業は explicit approval 後の repo-external wrapper install + mismatch drill |
| BL-0083 | snapshot state repo 外移動 (`~/.claude-trusted-state/taskmanagedai/`) | 7 → Phase 5 | 2026-05-24 helper PR で `TASKMANAGEDAI_HOOK_STATE_DIR` support + temp state tests を追加済。残作業は explicit approval 後の trusted-state directory 作成 + migration note |
| BL-0084 | sha256 manifest 生成 / 検証 | 7 → Phase 5 | 2026-05-24 helper PR で `scripts/regenerate-hook-manifest.sh` / `scripts/verify-hook-trust-root.sh` と temp manifest tests を追加済。残作業は trusted manifest install + wrapper self-test |
| ADR-00012 | Hook Trust Boundary | — | accepted 2026-05-22。実装 closeout は BL-0082/0083/0084 の external install evidence 後に記録 |
