...
|---|---|---|---|---|
| BL-0096a | RepoProxy 4 整合 binding server-side 再計算 refactor | 8 → 11 | 2026-05-24 Batch A で public signature は `DraftPRBinding` のみに閉鎖済。残作業は DB / ContextSnapshot / Git ref から 4 hash を再計算する DB-backed resolver | `tests/repoproxy/test_repoproxy_binding.py` (signature + 4-binding negative cases) + DB-backed resolver tests |
