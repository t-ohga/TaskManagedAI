...
| BL-0100 | AgentRunEvent `repo_pr_opened` actual emission | 8 → 11 | SP-008 | 2026-05-24 Batch D で `RepoPROpenedEventWriter` + append-only DB persistence + raw-token-free payload schema を追加済。残作業は final RepoProxy Draft PR execution path から writer への automatic call-site wiring | `tests/repoproxy/test_repo_pr_opened_event.py` |
