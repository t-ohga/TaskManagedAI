...
| SP0095-UX-01 | Newcomer Path first-use flow | 9 → 9.5 → partial | SP-009-5_p0_ui_deferred_surfaces | 2026-05-24 Batch F0 で contract plan を固定し、Batch F1 で read-only `/onboarding` route、Batch F2a で response-only dry-run intake API contract、Batch F2b で `/api/v1/onboarding/dry_run_plan` backend API、Batch F3 で dry-run plan-review UI、2026-05-25 Batch F4 で `tm` canonical CLI onboarding parity を実装済。F5 closeout は別 gate | first-run mutating AgentRun 禁止、`tm` canonical、no raw secret/log/payload exposure |
