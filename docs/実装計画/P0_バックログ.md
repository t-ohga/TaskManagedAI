...
| BL-0101a | Webhook HMAC SecretBroker-mediated service layer + Redis SETNX replay + rotation status 検証 | 8 → 11 | 2026-05-24 Batch C で `GitHubWebhookVerifier` service boundary、SecretBroker resolver protocol、Redis SETNX replay protocol、rotation status validation、redacted audit payload を追加済。残作業は concrete SecretBroker resolver / Redis adapter / FastAPI route wiring | `tests/repoproxy/test_webhook_service.py` |
