...
| BL-0101a | Webhook HMAC SecretBroker-mediated service layer + Redis SETNX replay + rotation status 検証 | 8 → 11 | `verify_github_webhook(request, secret_ref_current, secret_ref_previous, delivery_id)` 公開境界、`secret_refs.status` in `(active, deprecated)` 検証 | `tests/repoproxy/test_webhook_service_layer.py` 新規 |
