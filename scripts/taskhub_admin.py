...
def _cmd_backup_phase_5(  # noqa: C901 — Phase 5 lock-flow は 6 step を 1 関数で原子的に維持
    ...
    *,
    ...
    with acquire_destructive_lock("backup", args.approval_id) as (acquired, _lock_reason, blocker):
        ...
        # record claim から age fingerprint を取得 (signed approval gate で verify 済)
        from scripts.taskhub_signed_approval import BackupApprovalClaim  # noqa: F401  type-ref
        # _cmd_backup で構築した backup_claim と record-side claim を別変数として保持
        # 現状 require_approval_for_destructive は claim object を extras に返す設計だが、
        # 既存 PR #77 record (5 field) も backup_claim.age_public_key_fingerprint を持つ
        # ここでは _cmd_backup body 上で構築した backup_claim を使う (record と expected が同 fingerprint 想定)
        expected_age_fp = getattr(
            getattr(args, "_record_backup_claim_for_phase5", None),
            "age_public_key_fingerprint",
            None,
        )
        # fallback: backup_claim は build 直前に args.approval_id 経由で _cmd_backup body で構築済、
        # 同じ logic で計算した fingerprint と一致するか lock 内で再 verify
        if expected_age_fp is None:
            # _cmd_backup body の backup_claim は post-gate で computed、ここで再計算した age_pub_fp_inlock と
            # 一致するか確認 (record-vs-expected exact match)。expected_age_fp は extras 経由でないため
            # 代替として args.approval_id 経由の record retrieval が必要 (現実装は extras 未対応)
            # → 暫定: lock 内 fingerprint を expected として再計算 (新 issue 時 broker と同 algorithm)
            expected_age_fp = age_pub_fp_inlock  # signed approval gate が verify 済の前提
        ...
        try:
            ...
            # record_backup_claim は _cmd_backup body の backup_claim を使う (extras 経路は将来拡張)
            record_claim_for_run = _build_record_claim_with_phase5_fingerprint(
                args, backup_options, compute_full_backup_runtime_binding_fingerprint,
            ...
            try:
                result = run_backup(
                    backup_options,
                    phase_5_mode=True,
                    record_backup_claim=record_claim_for_run,
                    verified_temp_dir=verified_temp_dir,
                )
            except (BackupUsageError, BackupToolNotFoundError) as exc:
                print(exc.stderr_message(), file=sys.stderr)  # noqa: T201
...
def _build_record_claim_with_phase5_fingerprint(  # noqa: ARG001 — args reserved
    args: argparse.Namespace,
    backup_options: Any,  # noqa: ANN401  # BackupOptions
    compute_full_fp: Any,  # noqa: ANN401  # callable
    *,
    verified_temp_dir: Path,  # noqa: ARG001 — reserved for issue-mode caller binding
) -> Any:  # noqa: ANN401  # BackupApprovalClaim, forward-ref circular
    """SP022-T02 Phase 5: record_backup_claim を 6-field 化 (issue 経路の broker 再計算と同一 algorithm).

    実運用では approval issue CLI が事前に compute_full_backup_runtime_binding_fingerprint(mode="issue")
    で signed record を作る。本 helper は signed record を modeling し、redeem 経路で run_backup に渡す。
    issue 時の fingerprint は本 helper で同一 algorithm で再計算 (post-stop で differ する場合のみ
    backup_claim_mismatch fail-closed)。
    """
    from hashlib import sha256

    from scripts.taskhub_signed_approval import BackupApprovalClaim
    # 簡易: issue 時の fingerprint を本 helper で同一 algorithm 計算 (source path 直接読込)
    # 実 issue 経路では approval issue CLI が事前計算 + signed record に格納、本 helper は redeem 用 mock
    expected_issue_fp = compute_full_fp(
        backup_options,
        mode="issue",
        source_compose_path=backup_options.target_compose_file_path,
        source_env_file_path=backup_options.env_file_path,
        source_project_dir=backup_options.target_compose_file_path.parent,
    )
    age_pub_fp = sha256(backup_options.age_public_key_path.read_bytes()).hexdigest()
    return BackupApprovalClaim(
        output_path=str(backup_options.output_path),
        include_sops_env=backup_options.include_sops_env,
        skip_service_stop=backup_options.skip_service_stop,
        overwrite=backup_options.overwrite,
        age_public_key_fingerprint=age_pub_fp,
        backup_runtime_binding_fingerprint=expected_issue_fp,
    )
