...
GateId = Literal["AC-HARD-02"]
MetricKey = Literal["secret_canary_no_leak"]
...
_REDACTION_ENTRY_KEYS = frozenset(
    {"redacted", "raw_value_present", "fingerprint_sha256", "pattern_hit_kind"}
)
_ALLOWED_PATTERN_HIT_KINDS = frozenset(
    {"canary_pattern", "provider_key_pattern", "secret_pattern", "none"}
)
_ALLOWED_EXPECTED_DECISIONS: tuple[ExpectedDecision, ...] = ("block", "allow")
_ALLOWED_AUDIT_EVENT_TYPES: frozenset[str] = frozenset(
    {
        "provider_blocked",
        "policy_decision_created",
        "runtime_blocked",
...
_AUDIT_EVENT_KEYS = frozenset(
    {"event_type", "decision", "reason_code", "pattern_hit_kind", "redacted"}
)
_ALLOWED_AUDIT_DECISIONS = frozenset({"deny", "allow"})
...
class PublicFixture:
    """Complete public_regression fixture with readable redaction expectations."""
    ...
    expected_pattern_hit_kind: str
    expected_audit_events: list[dict[str, Any]]
    expected_redaction: dict[str, dict[str, Any]]
...
def _read_json_object(path: Path) -> dict[str, Any]:
        ...
        for key, _ in pairs:
            if key in seen:
                raise ValueError(
                    ...
                    "AC-HARD-02 fixture must use canonical JSON without duplicate keys"
                )
            seen.add(key)
        return dict(pairs)
    ...
    def _strict_parse_constant(constant: str) -> Any:
        raise ValueError(
            ...
            "AC-HARD-02 fixture must not contain NaN / Infinity / -Infinity"
        )
...
def _coerce_gate_id(value: object, *, source_path: Path) -> GateId:
    if value != "AC-HARD-02":
        raise ValueError(f"{source_path}: gate_id must be AC-HARD-02.")
    return "AC-HARD-02"


def _coerce_kpi_id(value: object, *, source_path: Path) -> None:
    if value is not None:
        raise ValueError(f"{source_path}: kpi_id must be null for AC-HARD-02 hard gate fixtures.")
    return None


def _coerce_metric_key(value: object, *, source_path: Path) -> MetricKey:
    if value != "secret_canary_no_leak":
        raise ValueError(f"{source_path}: metric_key must be secret_canary_no_leak.")
    return "secret_canary_no_leak"


def _coerce_expected_decision(value: object, *, source_path: Path) -> ExpectedDecision:
    if value not in _ALLOWED_EXPECTED_DECISIONS:
        raise ValueError(
            f"{source_path}: expected_decision must be one of {_ALLOWED_EXPECTED_DECISIONS}."
        )
...
def _require_expected_audit_events(
    value: object,
    *,
    source_path: Path,
) -> list[dict[str, Any]]:
    if not isinstance(value, list):
        raise ValueError(f"{source_path}: expected_audit_events must be an array.")

    events: list[dict[str, Any]] = []
    for index, item in enumerate(value):
        if not isinstance(item, dict):
            raise ValueError(
                f"{source_path}: expected_audit_events[{index}] must be a JSON object."
            )

        unknown_keys = sorted(set(item) - _AUDIT_EVENT_KEYS)
        if unknown_keys:
            raise ValueError(
                f"{source_path}: expected_audit_events[{index}] has unknown keys "
                f"{unknown_keys}; allowed keys are {sorted(_AUDIT_EVENT_KEYS)}"
            )

        event_type = item.get("event_type")
        if event_type not in _ALLOWED_AUDIT_EVENT_TYPES:
            raise ValueError(
                f"{source_path}: expected_audit_events[{index}].event_type must be one of "
                f"{sorted(_ALLOWED_AUDIT_EVENT_TYPES)}."
            )

        redacted = item.get("redacted")
        if redacted is not True:
            raise ValueError(
                f"{source_path}: expected_audit_events[{index}].redacted must be true."
            )

        decision = item.get("decision")
        if decision is not None and decision not in _ALLOWED_AUDIT_DECISIONS:
            raise ValueError(
                f"{source_path}: expected_audit_events[{index}].decision must be one of "
                f"{sorted(_ALLOWED_AUDIT_DECISIONS)}."
            )

        reason_code = item.get("reason_code")
        if reason_code is not None and (not isinstance(reason_code, str) or not reason_code):
            raise ValueError(
                f"{source_path}: expected_audit_events[{index}].reason_code "
                "must be a non-empty string when present."
            )

        pattern_hit_kind = item.get("pattern_hit_kind")
        if pattern_hit_kind is not None and pattern_hit_kind not in _ALLOWED_PATTERN_HIT_KINDS:
            raise ValueError(
                f"{source_path}: expected_audit_events[{index}].pattern_hit_kind must be one of "
                f"{sorted(_ALLOWED_PATTERN_HIT_KINDS)}."
            )
...
def _validate_redaction_contract(
    expected_redaction: dict[str, Any],
    *,
    expected_pattern_hit_kind: str,
    source_path: Path,
) -> dict[str, dict[str, Any]]:
    actual_paths = set(expected_redaction)
    missing = sorted(_REQUIRED_REDACTION_PATHS - actual_paths)
    unknown = sorted(actual_paths - _REQUIRED_REDACTION_PATHS)
    if missing or unknown:
        raise ValueError(
            f"{source_path}: expected_redaction paths mismatch; "
            f"missing={missing}, unknown={unknown}, required={sorted(_REQUIRED_REDACTION_PATHS)}"
        )

    expected_redacted = expected_pattern_hit_kind != "none"
    normalized: dict[str, dict[str, Any]] = {}
    for path_name in sorted(_REQUIRED_REDACTION_PATHS):
        entry = _require_dict(
            expected_redaction.get(path_name),
            source_path=source_path,
            key=f"expected_redaction.{path_name}",
        )
        unknown_entry_keys = sorted(set(entry) - _REDACTION_ENTRY_KEYS)
        missing_entry_keys = sorted(_REDACTION_ENTRY_KEYS - set(entry))
        if unknown_entry_keys or missing_entry_keys:
            raise ValueError(
                f"{source_path}: expected_redaction.{path_name} key mismatch; "
                f"missing={missing_entry_keys}, unknown={unknown_entry_keys}"
            )
        if entry.get("redacted") is not expected_redacted:
            expected_text = str(expected_redacted).lower()
            raise ValueError(
                f"{source_path}: expected_redaction.{path_name}.redacted must be {expected_text}"
            )
        if entry.get("raw_value_present") is not False:
            raise ValueError(
                f"{source_path}: expected_redaction.{path_name}.raw_value_present must be false"
            )
        fingerprint = entry.get("fingerprint_sha256")
        if not isinstance(fingerprint, str) or not _SHA256_PATTERN.fullmatch(fingerprint):
            raise ValueError(
                f"{source_path}: expected_redaction.{path_name}.fingerprint_sha256 "
                "must be a 64-char lowercase SHA-256 hex string"
            )
        pattern_hit_kind = entry.get("pattern_hit_kind")
        if pattern_hit_kind != expected_pattern_hit_kind:
            raise ValueError(
                f"{source_path}: expected_redaction.{path_name}.pattern_hit_kind "
                f"{pattern_hit_kind!r} != expected_pattern_hit_kind {expected_pattern_hit_kind!r}"
            )
        normalized[path_name] = dict(entry)
    return normalized
...
def _public_fixture_from_data(
    ...
    *,
    ...
    if (expected_decision == "block") != expected_block:
        raise ValueError(
            f"{source_path}: expected_block must be true when expected_decision is block "
            "and false when expected_decision is allow."
        )
    ...
    if expected_decision == "allow" and expected_reason_code != "allow":
        raise ValueError(f"{source_path}: allow fixture expected_reason_code must be allow.")
    if expected_decision == "block" and expected_reason_code != "provider_request_preflight_violation":
        raise ValueError(
            f"{source_path}: block fixture expected_reason_code must be "
            "provider_request_preflight_violation."
        )

    expected_pattern_hit_kind = _require_str(
        data.get("expected_pattern_hit_kind"),
        source_path=source_path,
        key="expected_pattern_hit_kind",
    )
    if expected_pattern_hit_kind not in _ALLOWED_PATTERN_HIT_KINDS:
        raise ValueError(
            f"{source_path}: expected_pattern_hit_kind must be one of "
            f"{sorted(_ALLOWED_PATTERN_HIT_KINDS)}."
        )
    if expected_decision == "allow" and expected_pattern_hit_kind != "none":
        raise ValueError(f"{source_path}: allow fixture expected_pattern_hit_kind must be none.")
    if expected_decision == "block" and expected_pattern_hit_kind == "none":
        raise ValueError(f"{source_path}: block fixture expected_pattern_hit_kind must not be none.")

    expected_redaction = _validate_redaction_contract(
        _require_dict(
            data.get("expected_redaction"),
            source_path=source_path,
            key="expected_redaction",
        ),
    ...
    return PublicFixture(
        ...
        expected_blocked_reason=_require_str(
            data.get("expected_blocked_reason"),
            source_path=source_path,
            key="expected_blocked_reason",
        ),
        ...
        expected_pattern_hit_kind=expected_pattern_hit_kind,
        expected_audit_events=_require_expected_audit_events(
            data.get("expected_audit_events"),
            source_path=source_path,
        ),
...
def load_manifest(manifest_path: Path) -> dict[str, Any]:
    """Load and validate AC-HARD-02 manifest.json."""

    manifest = dict(_read_json_object(manifest_path))
    hard_gate_id = manifest.get("hard_gate_id", manifest.get("gate_id"))
    if hard_gate_id != "AC-HARD-02":
        raise ValueError("manifest hard_gate_id must be AC-HARD-02.")

    metric = manifest.get("metric", manifest.get("metric_key"))
    if metric != "secret_canary_no_leak":
        raise ValueError("manifest metric must be secret_canary_no_leak.")
    ...
    manifest["gate_id"] = "AC-HARD-02"
    manifest["metric_key"] = "secret_canary_no_leak"
...
def load_public_regression_fixtures(
    base_path: Path = Path("eval/security/secret_canary"),
) -> list[PublicFixture]:
...
def assert_anti_gaming_invariants(
    ...
    """Enforce AC-HARD-02 fixture anti-gaming invariants."""
    ...
    for fixture in fixtures:
        ...
        if fixture.gate_id != "AC-HARD-02":
            raise ValueError(f"{fixture.fixture_id}: gate_id must be AC-HARD-02.")

        if fixture.kpi_id is not None:
            raise ValueError(f"{fixture.fixture_id}: kpi_id must be None for AC-HARD-02.")

        if fixture.metric_key != "secret_canary_no_leak":
            raise ValueError(
                f"{fixture.fixture_id}: metric_key must be secret_canary_no_leak."
            )
        ...
        if isinstance(fixture, PublicFixture):
            payload["expected_redaction"] = fixture.expected_redaction
            payload["expected_audit_events"] = fixture.expected_audit_events
            if set(fixture.expected_redaction) != _REQUIRED_REDACTION_PATHS:
                raise ValueError(
                    f"{fixture.fixture_id}: expected_redaction must cover all AC-HARD-02 "
                    f"surfaces {sorted(_REQUIRED_REDACTION_PATHS)}."
                )
        ...
        if isinstance(fixture, RedactedFixture):
            ...
            if leaks:
                raise ValueError(
                    ...
                    "never expose expected decisions, assertions, redaction paths, or leak payloads."
                )
