...
GateId = Literal["AC-HARD-03"]
MetricKey = Literal["tenant_isolation_negative_pass"]
ExpectedDecision = Literal["block", "allow"]
Fixture = "PublicFixture | RedactedFixture"
...
# Raw secret key names prohibited in Hard Gate fixture metadata/input.
# Keep this in sync with secret_refs_ck_metadata_no_raw_secret.
...
def _read_json_object(path: Path) -> dict[str, Any]:
    """fixture / manifest JSON を strict parse して dict を返す。

    F-027 (R24): duplicate key と NaN/Infinity を fail-closed で reject:
    - duplicate key: object_pairs_hook で同一 object 内の重複 key を ValueError
    - NaN/Infinity/-Infinity: parse_constant で ValueError
    """
    if not path.is_file():
    ...
    def _strict_object_pairs_hook(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        """同一 object 内に duplicate key があれば fail-closed reject。"""
        seen: set[str] = set()
        for key, _ in pairs:
            if key in seen:
                raise ValueError(
                    f"duplicate JSON object key {key!r} in {path}; "
                    f"AC-HARD-03 fixture must use canonical JSON without duplicate keys"
                )
            seen.add(key)
        return dict(pairs)

    def _strict_parse_constant(constant: str) -> Any:
        """NaN / Infinity / -Infinity を fail-closed reject。"""
        raise ValueError(
            f"non-canonical JSON constant {constant!r} in {path}; "
            f"AC-HARD-03 fixture must not contain NaN / Infinity / -Infinity"
        )
...
def _coerce_gate_id(value: object, *, source_path: Path) -> GateId:
    if value != "AC-HARD-03":
        raise ValueError(f"{source_path}: gate_id must be AC-HARD-03.")
    return "AC-HARD-03"


def _coerce_metric_key(value: object, *, source_path: Path) -> MetricKey:
    if value != "tenant_isolation_negative_pass":
        raise ValueError(f"{source_path}: metric_key must be tenant_isolation_negative_pass.")
    return "tenant_isolation_negative_pass"
...
def _require_optional_str(value: object, *, source_path: Path, key: str) -> str | None:
    if value is None:
        return None
    if not isinstance(value, str) or not value:
        raise ValueError(f"{source_path}: {key} must be null or a non-empty string.")
    return value
...
def _find_prohibited_keys_recursive(
    ...
    """Find prohibited keys at any depth and return jsonpath-style paths.

    F-021: tuple is scanned like list. Non-JSON container/object values are
    rejected fail-closed because redacted fixture attributes must not carry
    hidden expectation channels.
    F-023: dict keys are also constrained to strings so directly constructed
    fixture attributes remain JSON-compatible.
    """
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            if not isinstance(key, str):
                raise TypeError(
                    ...
                    f"redacted fixture attributes must be JSON-compatible "
                    f"(dict keys must be strings)"
                )
        ...
        raise TypeError(
            ...
            f"redacted fixture attributes must be JSON-compatible "
            f"(dict, list, tuple, str, int, float, bool, None)"
        )
    return leaks
...
def _assert_no_raw_secret_keys(
    ...
    """Reject raw secret key names from fixture metadata/input fail-closed.

    F-024 implements the BL-0030 invariant at the loader boundary.
    """
    ...
    if leaks:
        raise ValueError(
            ...
            f"absent from fixture metadata/input "
            f"(prohibited: {sorted(_PROHIBITED_SECRET_METADATA_KEYS)})"
        )


def _canonical_fixture_hash(data: dict[str, Any]) -> str:
    """NFC UTF-8 + JCS-like canonical JSON + SHA-256.

    F-027 (R24): allow_nan=False で hash 計算時も NaN/Infinity を reject。
    """
...
def _validate_fixture_immutable_index(manifest: dict[str, Any]) -> None:
    """Validate every fixture_immutable_index entry fail-closed."""
    ...
    for fixture_id, entry in index.items():
        if not isinstance(fixture_id, str) or not fixture_id:
            raise ValueError(
                f"manifest.fixture_immutable_index key must be a non-empty string: "
        ...
        if not _DATE_PATTERN.fullmatch(created_at):
            raise ValueError(
                ...
                f"YYYY-MM-DD with ASCII digits"
            )
        try:
...
def _assert_fixture_matches_immutable_index(
    ...
    if entry is None:
        raise ValueError(
            ...
            f"(append-only requires manifest registration before fixture file is added)"
        )
    if not isinstance(entry, dict):
...
def _assert_immutable_index_split_consistency(
    ...
    """Ensure manifest fixture registrations and discovered fixture IDs match exactly."""
    ...
    if missing_in_split:
        raise ValueError(
            ...
            f"manifest registrations exactly (deletion forbidden)."
        )
...
def _validate_fixture_against_schema(
    ...
    """Validate public fixtures against expected_schema.json without leaking values.

    F-016: pass FormatChecker so `format: date` is enforced without including raw
    instance values in the raised ValueError.
    """
...
def _resolve_split_dir(base_path: Path, kind: str, manifest: dict[str, Any]) -> Path:
    """Resolve a fixture split path while keeping it under the fixture root.

    F-029 (R28): fail closed for absolute paths, `..` traversal,
    non-canonical split directory names, and symlink/resolve escapes.
    """
...
def _resolve_expected_schema_path(base_path: Path, manifest: dict[str, Any]) -> Path:
    """manifest.expected_schema の path を fail-closed で fixed canonical filename に強制。

    F-032 (R30): absolute path / `..` traversal / alternate filename /
    base_path 外 resolve を reject。
    """
...
def _public_fixture_from_data(
    ...
    *,
    ...
    return PublicFixture(
        ...
        expected_failure=_require_optional_str(
            data.get("expected_failure"),
            source_path=source_path,
    ...
    *,
    ...
    if unknown_keys:
        raise ValueError(
            ...
            f"Anti-gaming invariant requires redacted fixtures to use only the allowed schema."
        )
...
def load_manifest(manifest_path: Path) -> dict[str, Any]:
    """Load and validate AC-HARD-03 manifest.json."""

    manifest = dict(_read_json_object(manifest_path))
    hard_gate_id = manifest.get("hard_gate_id", manifest.get("gate_id"))
    if hard_gate_id != "AC-HARD-03":
        raise ValueError("manifest hard_gate_id must be AC-HARD-03.")

    metric = manifest.get("metric", manifest.get("metric_key"))
    if metric != "tenant_isolation_negative_pass":
        raise ValueError("manifest metric must be tenant_isolation_negative_pass.")
    ...
    manifest["gate_id"] = "AC-HARD-03"
    manifest["metric_key"] = "tenant_isolation_negative_pass"
    ...
    if expectation_leaks:
        raise ValueError(
            ...
            f"manifest must not include redacted expectation keys "
            f"(prohibited: {sorted(_PROHIBITED_REDACTED_KEYS)}). "
            f"Anti-gaming requires private_holdout/adversarial_new expectations to be "
            f"absent from manifest."
        )
    ...
    if secret_leaks:
        raise ValueError(
            ...
            f"manifest must not include raw secret-related keys "
            f"(prohibited: {sorted(_PROHIBITED_SECRET_METADATA_KEYS)}). "
            f"DB and loader denylist must be in sync."
        )
...
def load_expected_schema(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"expected_schema is not a valid Draft 7 schema: {path}: {exc.message}") from exc
...
def load_public_regression_fixtures(
    base_path: Path = Path("eval/security/tenant_isolation"),
) -> list[PublicFixture]:
...
def assert_anti_gaming_invariants(
    ...
    """Enforce AC-HARD-03 fixture anti-gaming invariants."""
        ...
        if isinstance(fixture, PublicFixture):
            if fixture.fixture_kind != "public_regression":
                raise ValueError(
                    ...
                    f"fixture_kind='public_regression'"
                )
            ...
            if fixture.fixture_kind not in {"private_holdout", "adversarial_new"}:
                raise ValueError(
                    ...
                    f"fixture_kind in {{'private_holdout', 'adversarial_new'}}"
                )
        else:
            raise TypeError(
                f"unsupported fixture type {type(fixture).__name__}; "
                f"expected PublicFixture or RedactedFixture"
            )
    ...
    for fixture in fixtures:
        ...
        if fixture.gate_id != "AC-HARD-03":
            raise ValueError(f"{fixture.fixture_id}: gate_id must be AC-HARD-03.")

        if fixture.metric_key != "tenant_isolation_negative_pass":
            raise ValueError(
                f"{fixture.fixture_id}: metric_key must be tenant_isolation_negative_pass."
            )
        ...
        for field_name, field_value in (("metadata", fixture.metadata), ("input", fixture.input)):
            ...
            if secret_leaks:
                raise ValueError(
                    ...
                    f"absent from fixture attributes (post-load check)"
                )
        ...
        if isinstance(fixture, RedactedFixture):
            ...
            if leaks:
                raise ValueError(
                    ...
                    f"Anti-gaming invariant requires redacted fixture attributes to "
                    f"never expose expected_decision/expected_failure/expected_reason_code/"
                    f"pattern_hit_kind/assertions even after construction."
                )
