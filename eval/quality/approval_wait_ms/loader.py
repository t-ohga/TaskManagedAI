...
KpiId = Literal["AC-KPI-03"]
MetricKey = Literal["approval_wait_ms"]
...
_PROHIBITED_REDACTED_KEYS = frozenset({"expected_aggregate", "assertions"})
...
def _read_json_object(path: Path) -> dict[str, Any]:
        ...
        for key, _ in pairs:
            if key in seen:
                raise ValueError(
                    ...
                    "AC-KPI-03 fixture must use canonical JSON without duplicate keys"
                )
            seen.add(key)
        return dict(pairs)
    ...
    def _strict_parse_constant(constant: str) -> Any:
        raise ValueError(
            ...
            "AC-KPI-03 fixture must not contain NaN / Infinity / -Infinity"
        )
...
def _coerce_kpi_id(value: object, *, source_path: Path) -> KpiId:
    if value != "AC-KPI-03":
        raise ValueError(f"{source_path}: kpi_id must be AC-KPI-03.")
    return "AC-KPI-03"


def _coerce_metric_key(value: object, *, source_path: Path) -> MetricKey:
    if value != "approval_wait_ms":
        raise ValueError(f"{source_path}: metric_key must be approval_wait_ms.")
    return "approval_wait_ms"
...
def _percentile_cont(sorted_values: list[float], q: float) -> float:
    """PostgreSQL percentile_cont equivalent linear interpolation."""
...
def _compute_expected_aggregate(sample_approvals: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute wait_ms from approved/rejected samples with decided_at present."""

    decided_durations_ms: list[float] = []
    for sample_approval in sample_approvals:
        if not isinstance(sample_approval, dict):
            continue

        status = sample_approval.get("status")
        if status not in {"approved", "rejected"}:
            continue

        requested_at_str = sample_approval.get("requested_at")
        decided_at_str = sample_approval.get("decided_at")
        if not isinstance(requested_at_str, str) or not isinstance(decided_at_str, str):
            continue

        requested_at = _datetime.datetime.fromisoformat(
            requested_at_str.replace("Z", "+00:00")
        )
        decided_at = _datetime.datetime.fromisoformat(decided_at_str.replace("Z", "+00:00"))
        if decided_at < requested_at:
            continue

        ms = (decided_at - requested_at).total_seconds() * 1000.0
        decided_durations_ms.append(ms)
    ...
    sorted_ms = sorted(decided_durations_ms)
    median_ms = _percentile_cont(sorted_ms, 0.5)
    p95_ms = _percentile_cont(sorted_ms, 0.95)
    return {
...
def _validate_aggregate_consistency(
    ...
    """Fail closed when expected_aggregate drifts from input.sample_approvals."""

    sample_approvals = fixture.input.get("sample_approvals", [])
    if not isinstance(sample_approvals, list):
        return

    computed = _compute_expected_aggregate(sample_approvals)
    expected = fixture.expected_aggregate

    if computed["sample_count"] != expected.get("sample_count"):
        raise ValueError(
            f"expected_aggregate.sample_count mismatch in {source_path.name}: "
            f"computed {computed['sample_count']}, fixture {expected.get('sample_count')}"
        )

    for field in ("median_ms", "p95_ms", "min_ms", "max_ms"):
        ...
        if abs(computed_value - float(expected_value)) > 1.0:
            raise ValueError(
                f"expected_aggregate.{field} mismatch in {source_path.name}: "
                f"computed {computed_value}, fixture {expected_value} (diff > 1ms)"
            )
...
def load_manifest(manifest_path: Path) -> dict[str, Any]:
    """Load and validate AC-KPI-03 manifest.json."""

    manifest = dict(_read_json_object(manifest_path))
    if manifest.get("kpi_id") != "AC-KPI-03":
        raise ValueError("manifest kpi_id must be AC-KPI-03.")

    metric = manifest.get("metric", manifest.get("metric_key"))
    if metric != "approval_wait_ms":
        raise ValueError("manifest metric must be approval_wait_ms.")
    ...
    manifest["metric_key"] = "approval_wait_ms"
...
def load_public_regression_fixtures(
    base_path: Path = Path("eval/quality/approval_wait_ms"),
) -> list[PublicFixture]:
...
def assert_anti_gaming_invariants(
    ...
    """Enforce AC-KPI-03 fixture anti-gaming invariants."""
    ...
    for fixture in fixtures:
        ...
        if fixture.kpi_id != "AC-KPI-03":
            raise ValueError(f"{fixture.fixture_id}: kpi_id must be AC-KPI-03.")

        if fixture.metric_key != "approval_wait_ms":
            raise ValueError(f"{fixture.fixture_id}: metric_key must be approval_wait_ms.")
