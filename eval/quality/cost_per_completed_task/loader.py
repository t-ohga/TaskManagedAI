...
KpiId = Literal["AC-KPI-05"]
MetricKey = Literal["cost_per_completed_task"]
...
_DEFAULT_THRESHOLD_USD = Decimal("0.5")
...
def _read_json_object(path: Path) -> dict[str, Any]:
        ...
        for key, _ in pairs:
            if key in seen:
                raise ValueError(
                    ...
                    "AC-KPI-05 fixture must use canonical JSON without duplicate keys"
                )
            seen.add(key)
        return dict(pairs)
    ...
    def _strict_parse_constant(constant: str) -> Any:
        raise ValueError(
            ...
            "AC-KPI-05 fixture must not contain NaN / Infinity / -Infinity"
        )
...
def _manifest_dataset_version_id(manifest: dict[str, Any]) -> str:
    ...
    if not isinstance(dataset_version_id, str) or not dataset_version_id:
        raise ValueError("manifest dataset_version_id must be a non-empty string.")
...
def _coerce_kpi_id(value: object, *, source_path: Path) -> KpiId:
    if value != "AC-KPI-05":
        raise ValueError(f"{source_path}: kpi_id must be AC-KPI-05.")
    return "AC-KPI-05"


def _coerce_metric_key(value: object, *, source_path: Path) -> MetricKey:
    if value != "cost_per_completed_task":
        raise ValueError(f"{source_path}: metric_key must be cost_per_completed_task.")
    return "cost_per_completed_task"
...
def _validate_no_raw_secret_values(data: Any, source_path: Path) -> None:
    leaks = _find_raw_secret_value_patterns_recursive(data)
    if leaks:
        raise ValueError(
            f"{source_path}: raw secret value pattern detected at {sorted(leaks)}; "
            "raw values are never echoed in loader errors"
        )
...
def _compute_expected_aggregate(sample_runs: list[dict[str, Any]]) -> dict[str, Any]:
    """Compute completed-run cost KPI from terminal AgentRun samples."""

    total_cost = Decimal("0")
    completed_count = 0
    for sample_run in sample_runs:
        if not isinstance(sample_run, dict):
            continue
        if sample_run.get("status") != "completed":
            continue

        cost = _to_decimal(sample_run.get("cost_usd"))
        if cost is None:
            continue
    ...
    total_cost_float = _money_to_float(total_cost)
    if completed_count == 0:
        cost_per_completed: float | None = None
        threshold_passed = False
    else:
        cost_per_completed = _money_to_float(total_cost / Decimal(completed_count))
        threshold_passed = Decimal(str(cost_per_completed)) <= _DEFAULT_THRESHOLD_USD

    return {
        "total_completed_runs": completed_count,
        "total_cost_usd": total_cost_float,
        "cost_per_completed_task_usd": cost_per_completed,
        "threshold_usd": float(_DEFAULT_THRESHOLD_USD),
...
def _validate_aggregate_consistency(
    ...
    """Fail closed when expected_aggregate drifts from input.sample_runs."""

    sample_runs = fixture.input.get("sample_runs", [])
    if not isinstance(sample_runs, list):
        return

    computed = _compute_expected_aggregate(sample_runs)
    expected = fixture.expected_aggregate

    if computed["total_completed_runs"] != expected.get("total_completed_runs"):
        raise ValueError(
            f"expected_aggregate.total_completed_runs mismatch in {source_path.name}: "
            f"computed {computed['total_completed_runs']}, "
            f"fixture {expected.get('total_completed_runs')}"
        )

    for field in ("total_cost_usd", "cost_per_completed_task_usd", "threshold_usd"):
        ...
        if computed_value is None or expected_value is None:
            raise ValueError(
                f"expected_aggregate.{field} mismatch in {source_path.name}: "
                f"computed {computed_value}, fixture {expected_value}"
            )
        ...
        if abs(float(computed_value) - float(expected_value)) > 0.000001:
            raise ValueError(
                f"expected_aggregate.{field} mismatch in {source_path.name}: "
                f"computed {computed_value}, fixture {expected_value} (diff > 0.000001)"
            )

    if computed["threshold_passed"] is not expected.get("threshold_passed"):
        raise ValueError(
            f"expected_aggregate.threshold_passed mismatch in {source_path.name}: "
            f"computed {computed['threshold_passed']}, "
            f"fixture {expected.get('threshold_passed')}"
        )
...
def _public_fixture_from_data(
    ...
    *,
    ...
    _validate_no_raw_secret_values(data, source_path)
...
def load_manifest(manifest_path: Path) -> dict[str, Any]:
    """Load and validate AC-KPI-05 manifest.json."""

    manifest = dict(_read_json_object(manifest_path))
    _validate_no_raw_secret_values(manifest, manifest_path)
    if manifest.get("kpi_id") != "AC-KPI-05":
        raise ValueError("manifest kpi_id must be AC-KPI-05.")

    metric = manifest.get("metric", manifest.get("metric_key"))
    if metric != "cost_per_completed_task":
        raise ValueError("manifest metric must be cost_per_completed_task.")

    threshold = manifest.get("threshold")
    if not isinstance(threshold, dict):
        raise ValueError("manifest threshold must be a JSON object.")
    max_cost = threshold.get("cost_per_completed_task_usd_max")
    if not isinstance(max_cost, int | float) or isinstance(max_cost, bool) or max_cost != 0.5:
        raise ValueError("manifest threshold.cost_per_completed_task_usd_max must be 0.5.")
    if threshold.get("currency") != "USD":
        raise ValueError("manifest threshold.currency must be USD.")
    ...
    manifest["metric_key"] = "cost_per_completed_task"
...
def load_public_regression_fixtures(
    base_path: Path = Path("eval/quality/cost_per_completed_task"),
) -> list[PublicFixture]:
...
def assert_anti_gaming_invariants(
    ...
    """Enforce AC-KPI-05 fixture anti-gaming invariants."""
    ...
    for fixture in fixtures:
        ...
        if fixture.kpi_id != "AC-KPI-05":
            raise ValueError(f"{fixture.fixture_id}: kpi_id must be AC-KPI-05.")

        if fixture.metric_key != "cost_per_completed_task":
            raise ValueError(
                f"{fixture.fixture_id}: metric_key must be cost_per_completed_task."
            )
        ...
        for field_name, field_value in (("metadata", fixture.metadata), ("input", fixture.input)):
            ...
            value_leaks = _find_raw_secret_value_patterns_recursive(field_value)
            if value_leaks:
                raise ValueError(
                    f"fixture {fixture.fixture_id} {field_name} contains raw secret value "
                    f"patterns at {sorted(value_leaks)}; raw values are never echoed "
                    "in loader errors (post-load check)"
                )
        ...
        if isinstance(fixture, RedactedFixture):
            ...
            if leaks:
                raise ValueError(
                    ...
                    "never expose expected_aggregate/assertions even after construction."
                )
