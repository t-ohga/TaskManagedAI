...
- [ ] **Codex PR review で「rule 違反検出漏れ」がない**: `<stage1_sha>` から `HEAD` までの全 PR (`gh pr list --base main --state merged --search "merged:>=<stage1_sha_date>"`) について `.claude/scripts/codex_pr_full_review.sh <PR>` で全 finding を確認、rule 違反指摘で本来 stage 1 で load されるべき rule が漏れて発生した finding が 0 件
...
# B1-B7 Hard Gates 7 (各 Hard Gate は最低 1 つの L1 rule で hit)
for gate in 'policy_block_recall' 'secret_canary_no_leak|secret canary' 'tenant_isolation_negative_pass|tenant_id' \
  ...
  count=$(grep -rlE "$gate" .claude/rules/ 2>/dev/null | xargs -I{} grep -lE "$gate" {} | wc -l)
  echo "[$count] Hard Gate: $gate"
  [ "$count" -eq 0 ] && { echo "VIOLATION: Hard Gate '$gate' は L1 rule で hit せず"; exit 1; }
done
...
|---:|---|---:|---|---|---|---:|---:|
| 1 | `core.md` | 3.2k | L1 | 維持 | E (圧縮) | 3.0k | 2.5k |
| 2 | `ai-output-boundary.md` | 2.8k | L1 | 維持 | E (圧縮) | 2.5k | 2.0k |
| 3 | `instincts.md` | 3.2k | L1 | 維持 | E (圧縮) | 2.8k | 2.3k |
| 4 | `secretbroker-boundary.md` | 4.7k | L1 | 維持 | E (圧縮) | 3.5k | 3.0k |
| 5 | `provider-compliance.md` | 3.6k | L1 | 維持 | E (圧縮) | 3.0k | 2.5k |
| 6 | `agentrun-state-machine.md` | 4.6k | L1 | 維持 | E (圧縮) | 3.0k | 2.5k |
| 7 | `cross-source-enum-integrity.md` | 1.7k | L1 | 維持 | - | 1.5k | 1.5k |
| 8 | `server-owned-boundary.md` | 1.3k | L1 | 維持 | - | 1.2k | 1.2k |
| 9 | `sprint-pack-adr-gate.md` | 3.0k | **L1 (F-CRA-002 fix)** | 維持 | E (圧縮) | 2.5k | 2.0k |
| 10 | `codex-usage-policy.md` | 2.6k | **L1 (F-CRA-007 fix)** | 維持 | E (圧縮) | 2.0k | 1.5k |
| 11 | `rendering.md` | 2.3k | L2 | `paths: ["frontend/**", "docs/基本設計/UI*.md", "docs/sprints/SP-009_*.md", "docs/sprints/SP-010_*.md"]` | B | 0 (条件 load) | 0 |
| 12 | `testing.md` | 3.5k | L2 | `paths: ["backend/**", "frontend/**", "migrations/**", "eval/**", "**/tests/**", "**/test_*.py", "**/*.spec.ts", "**/*.test.ts"]` (F-CRA-008 fix) | B | 0 (条件 load) | 0 |
| 13 | `code-search.md` | 2.4k | L2 | `paths: ["**/*.py", "**/*.ts", "**/*.tsx", "**/*.js", "**/*.jsx", "**/*.go", "**/*.rs", "**/*.sh"]` | B | 0 (条件 load) | 0 |
...
- 旧 rule への dead link 不在: `grep -r "rules/codex-multi-round\|rules/codex-output\|rules/multi-agent\|rules/codex-pr-review\|rules/user-preferences" .claude/ docs/` で **0 件**
