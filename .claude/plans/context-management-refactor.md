...
- **F-ADV-203 + F-ADV-301 fix**: rollback 正本は **git object** (`<phase-d-base-sha>` の immutable hash)、archive は **convenience cache に降格**。Phase D 前に archive を作成するが、**rollback の真正性検証は git object hash と比較**して行う:
  ```bash
  PHASE_D_BASE_SHA=$(git rev-parse HEAD)
  ARCHIVE_DIR=".claude/archived/${PHASE_D_BASE_SHA}-$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$ARCHIVE_DIR"
  # convenience cache (archive 内の manifest だけでは tamper 検証不可、git object が真正本)
  ...
  echo "WARN: archive is convenience cache only; restore TRUTH is git object at base_sha." >> "$ARCHIVE_DIR/restore-source.txt"
  ```
  - **rollback 時の真正性検証 (F-ADV-301 fix)**: archive 内 manifest だけでは self-referential (attacker が archive + manifest 両方書き換えで PASS = false green)、**`git show <phase-d-base-sha>:<path>` で計算した期待値と archive file の hash を比較**してから `.claude/rules/` に restore する
...
| D (F-ADV-203 + F-ADV-301 fix) | **`git revert <PR-D-merge-commit>` を第一選択**、git object (`<phase-d-base-sha>` immutable hash) が rollback の真正本。archive は convenience cache のみ、archive 経由 restore する場合は **`git show <phase-d-base-sha>:.claude/rules/<file>.md \| shasum -a 256` で計算した期待値と archive file の hash を比較**してから copy back (self-referential manifest だけでは tamper false green リスク)。`git revert` で復元できる場合は archive copy を実行しない。 | git revert 失敗時のみ archive 経路、必ず `git show <sha>:<path>` の hash 期待値と archive file hash を `diff` で照合、PASS のみ `cp $ARCHIVE_DIR/<file>.md .claude/rules/`、frontmatter 再検証 + `/context` 再計測 |
