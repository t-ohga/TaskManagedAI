...
|---|---|---|
| **CRITICAL invariant 直結 code** (CLAUDE.md §6.5.0 区別軸の 1 列目: AgentRun 16 状態 / SecretBroker / Provider Compliance / migration / approval decider human-only / 5+ source 整合 / DB schema) | **`Skill(skill="codex-all-loops", args="<target> --mode=code ...")`** (codex-impl-loop + codex-adversarial-loop + codex-review-loop の 3 phase chain) | impl-loop で実装、adversarial-loop で敵対視点 / security boundary、review-loop で総合磨き。**3 layer の review で抜け漏れを catch** (例: QL-A で 37 findings closure、Sprint 7-9 audit で 32 findings closure) |
| **新規 ADR / Sprint Pack / design doc 起票** (heavy plan、CRITICAL gate review) | **`Skill(skill="codex-all-loops", args="<target> --mode=plan ...")`** (codex-review-loop + codex-adversarial-loop の 2 phase chain) | review-loop で構造磨き、adversarial-loop で抜け漏れ catch (例: Wave 12 audit 28 round / 108 findings 全件 adopt) |
...
**起動方法の補足** (F-PR20-R1-002 P2 adopt 反映): `codex-all-loops` は **user-scope skill** (`~/.claude/skills/codex-all-loops/`) であり、project repository checkout には含まれていない。Claude 経由で起動する場合は `Skill(skill="codex-all-loops", args="...")` ツールを使用、user CLI 経由なら `/codex-all-loops` slash command (Claude Code session 内)。bare `codex-all-loops --mode=code` は CLI で直接 invoke できない (executable ではない、skill 名)。
...
3. **`verdict: clean` までループ**: 各 phase で `findings: []` (HIGH/CRITICAL は全件 adopt or 明示 reject 完了)、もしくは `CRITICAL=0 / HIGH=0 / new finding 0` (P3/info finding のみ explicit accept は OK) まで multi-round。F-PR20-R1-001 P2 adopt 反映: **HIGH finding を unresolved のまま clean 判定しない**、HIGH は全件 adopt / reject (rationale 付き) / defer (full defer_entry schema 記入済) のいずれかに closure 必要
