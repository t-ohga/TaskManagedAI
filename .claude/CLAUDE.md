...
- Secrets: SecretBroker、`secret_ref` (backend=`local`|`sops`、ADR-00058、Phase 0 default=`local` LocalSecretStore、SOPS+age 移行は D-4)、capability token
...
- `docs/基本設計/06_秘密管理設計.md`
  - DD-06。SecretBroker、secret backend (`local` Phase 0 default / `sops` D-4、ADR-00058)、material lifecycle、atomic claim、secret inventory、canary、rotation を確認する。
- `docs/実装計画/`
