...
- **`material_state='present'` 必須** (ADR-00058/00059)。`writing` (create/rotate 途中で store 未完了) / `purging` / `purged` は token 発行禁止 (false-present な未完了 material から token を出さない)。
...
select status, material_state, allowed_consumers, allowed_operations, scope
  from secret_refs
 where tenant_id = :tenant_id
   and id = :secret_ref_id
   for update;
-- status='active' (rotation.verify は status='pending' も可)
-- material_state='present' 必須 (writing/purging/purged は false-present として deny、ADR-00058/00059)
...
- 再検証で revoked / deprecated / scope mismatch / `material_state != 'present'` を検出したら raw secret を resolve しない。`secret_capability_denied` を記録（理由: secret_ref_revoked / scope_mismatch / consumer_mismatch / operation_mismatch / material_not_present）。
